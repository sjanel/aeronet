...
class Http2Connection {
 public:
  ...
  [[nodiscard]] const Http2Stream* getStream(uint32_t streamId) const noexcept;
 ...
 private:
  ...
  Http2Stream* createStream(uint32_t streamId);
  void closeStream(uint32_t streamId, ErrorCode errorCode = ErrorCode::NoError);
  ...
  // Stream management
  flat_hash_map<uint32_t, Http2Stream> _streams;
