...
RequestMiddleware BuildRateLimitMiddleware(RateLimitRequestMiddlewareBuilder options) {
  ...
  return [opts = std::move(options)](HttpRequestView& request) mutable {
    ...
    auto response =
        request.makeResponse(http::HeaderSize(kRetryAfterHeader.size(), nchars(decision.retryAfterSeconds)) +
                                 HttpResponse::BodySize(opts.rejectionBody.size()),
                             http::StatusCodeTooManyRequests);

    response.headerAddLine(http::RetryAfter, decision.retryAfterSeconds);
