...
std::size_t HpackDecoder::decodeHuffman(std::span<const std::byte> data) {
  ...
  while (byteIdx < data.size() || bitsInBuffer >= 5) {
    ...
    if (bitsInBuffer >= static_cast<int32_t>(kHuffmanLevel1Bits)) {
      ...
      if (entry.bitsUsed != 0 && entry.bitsUsed <= bitsInBuffer) {
    ...
    bool found = false;
    const int32_t startBits = (bitsInBuffer >= static_cast<int32_t>(kHuffmanLevel1Bits)) ? 10 : 5;
