...
namespace {
std::string httpRequest(uint16_t port, const std::string& method, const std::string& path, std::string body = {}) {
  ...
  while (true) {
    ...
    break;  // n == 0 (closed) or error (treat as done for test simplicity)
  }
  ::close(fd);
  return out;
}
...
TEST(HttpServerMixed, MixedPerPathHandlers) {
  ...
  std::thread th([&] { srv.runUntil([] { return false; }, std::chrono::milliseconds(5)); });
  std::this_thread::sleep_for(std::chrono::milliseconds(40));
  auto getResp = httpRequest(srv.port(), "GET", "/mix");
  auto decoded = extractBody(getResp);
  EXPECT_EQ(decoded, "STREAM");
  auto postResp = httpRequest(srv.port(), "POST", "/mix", "x");
...
TEST(HttpServerMixed, GlobalFallbackPrecedence) {
  ...
  std::thread th([&] { srv.runUntil([] { return false; }, std::chrono::milliseconds(5)); });
  std::this_thread::sleep_for(std::chrono::milliseconds(40));
  auto pathStreamResp = httpRequest(srv.port(), "GET", "/s");
  EXPECT_NE(std::string::npos, pathStreamResp.find("PS"));
  auto pathNormalResp = httpRequest(srv.port(), "GET", "/n");
  EXPECT_NE(std::string::npos, pathNormalResp.find("PN"));
  auto fallback = httpRequest(srv.port(), "GET", "/other");
...
TEST(HttpServerMixed, GlobalNormalOnlyWhenNoStreaming) {
  ...
  auto result = httpRequest(srv.port(), "GET", "/x");
...
TEST(HttpServerMixed, HeadRequestOnStreamingPathSuppressesBody) {
  ...
  std::thread th([&] { srv.runUntil([] { return false; }, std::chrono::milliseconds(5)); });
  std::this_thread::sleep_for(std::chrono::milliseconds(40));
  auto headResp = httpRequest(srv.port(), "HEAD", "/head");
...
TEST(HttpServerMixed, MethodNotAllowedWhenOnlyOtherStreamingMethodRegistered) {
  ...
  std::thread th([&] { srv.runUntil([] { return false; }, std::chrono::milliseconds(5)); });
  std::this_thread::sleep_for(std::chrono::milliseconds(40));
  auto postResp = httpRequest(srv.port(), "POST", "/m405", "data");
  ...
  auto getResp = httpRequest(srv.port(), "GET", "/m405");
  auto decoded = extractBody(getResp);
  EXPECT_EQ(decoded, "OKGET");
  srv.stop();
  th.join();
}
...
std::string twoRequestsKeepAlive(uint16_t port, const std::string& r1, const std::string& r2) {
  int fd = ::socket(AF_INET, SOCK_STREAM, 0);
  if (fd < 0) {
    return {};
  }
  ...
  std::string reqs = r1 + r2;  // simple sequential (not pipelined waiting) - we send both then read
  ::send(fd, reqs.data(), reqs.size(), 0);
  std::string out;
  char buf[8192];
  // Read until connection closed by server (or short sleep heuristic?) We'll rely on connection close after tests.
  // For deterministic behavior we ensure server closes after second due to enableKeepAlive=true and we only send two.
  while (true) {
    ssize_t bytesRead = ::recv(fd, buf, sizeof(buf), 0);
    if (bytesRead > 0) {
      out.append(buf, static_cast<std::size_t>(bytesRead));
...
TEST(HttpServerMixed, KeepAliveSequentialMixedStreamingAndNormal) {
  ...
  std::string r2 =
      "POST /ka HTTP/1.1\r\nHost: test\r\nConnection: close\r\nContent-Length: 0\r\n\r\n";  // normal, closes
  auto raw = twoRequestsKeepAlive(srv.port(), r1, r2);
