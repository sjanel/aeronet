...
PollTimeoutPolicy AdaptivePolicy() {
  return PollTimeoutPolicy{.baseTimeout = std::chrono::milliseconds{1}, .minFactor = 0.0F, .maxFactor = 64.0F};
}
...
void BM_FixedIdlePoll(benchmark::State& state) {
  ...
  EventLoop loop(PollTimeoutPolicy{.baseTimeout = std::chrono::milliseconds{1}, .minFactor = 0.0F, .maxFactor = 1.0F},
                 kBenchCapacity);
