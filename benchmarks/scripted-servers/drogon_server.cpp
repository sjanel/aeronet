...
int main(int argc, char* argv[]) {
  ...
  app.registerHandler(
      ...
      [](const drogon::HttpRequestPtr& req, std::function<void(const drogon::HttpResponsePtr&)>&& callback) {
        ...
        for (std::size_t headerPos = 0; headerPos < count; ++headerPos) {
          std::string headerName = std::format("X-Bench-Header-{}", headerPos);
          std::string headerValue = GenerateRandomString(headerSize);
  ...
  // ============================================================
  // Endpoint 9: /static/* - Static file serving
  // ============================================================
  if (!staticDir.empty()) {
    app.registerHandler(
        "/{file_path}",
        [staticDir]([[maybe_unused]] const drogon::HttpRequestPtr& req,
          ...
          auto resp = drogon::HttpResponse::newHttpResponse();
          resp->setStatusCode(drogon::k200OK);
          resp->addHeader("Content-Type", GetContentType(filePath));
