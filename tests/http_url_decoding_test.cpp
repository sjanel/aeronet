...
TEST(HttpUrlDecoding, SpaceDecoding) {
  ...
  auto respOwned = aeronet::test::requestOrThrow(ts.server.port(), optHello);
...
TEST(HttpUrlDecoding, Utf8Decoded) {
  ...
  ts.server.router().setPath(aeronet::http::Method::GET, decodedPath, [](const HttpRequest &) {
  ...
  auto respOwned = aeronet::test::requestOrThrow(ts.server.port(), optUtf8);
  std::string_view resp = respOwned;
  EXPECT_TRUE(resp.contains("200 OK"));
...
TEST(HttpUrlDecoding, PlusIsNotSpace) {
  ...
  optPlus.method = "GET";
  optPlus.target = "/a+b";
  auto respOwned = aeronet::test::requestOrThrow(ts.server.port(), optPlus);
  std::string_view resp = respOwned;
  EXPECT_TRUE(resp.contains("200 OK"));
...
TEST(HttpUrlDecoding, InvalidPercentSequence400) {
  ...
  auto respOwned = aeronet::test::requestOrThrow(ts.server.port(), optBad);
  std::string_view resp = respOwned;
...
TEST(HttpUrlDecoding, IncompletePercentSequence400) {
  ...
  opt.method = "GET";
  opt.target = "/bad%";
  auto resp = aeronet::test::requestOrThrow(ts.server.port(), opt);
  EXPECT_TRUE(resp.contains("400 Bad Request"));
}
...
TEST(HttpUrlDecoding, MixedSegmentsDecoding) {
  ...
  auto resp = aeronet::test::requestOrThrow(ts.server.port(), opt2);
  EXPECT_TRUE(resp.contains("200 OK"));
