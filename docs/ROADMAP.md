...
- **HTTP client response ownership** - suitably-sized HTTP/1.1 chunk reassembly and decompression outputs, plus assembled HTTP/2 identity and decompressed bodies, now rotate their scratch allocations into `HttpResponse`, preserving an equal-capacity replacement while removing the final body copy. Oversized scratch stays reusable and falls back to copying a small body, avoiding duplicate high-water retention. Identity bodies embedded in the HTTP/1.1 receive buffer still need a safe ownership-transfer or result-owned-buffer path. Cover identity, chunked, compressed, empty and 1 MiB+ responses. This complements the request-side wire-buffer unification already listed in the HTTP client section below.
