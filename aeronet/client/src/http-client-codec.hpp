...
// exact same bricks (RequestDecompressionState / ResponseCompressionState / HttpCodec).
struct HttpClientCodec {
  explicit HttpClientCodec(const CompressionConfig& compressionCfg) : compressionState(compressionCfg) {}

  RequestDecompressionState decompressionState;  // inbound response decoders
  ResponseCompressionState compressionState;     // outbound request encoders
