...
test::TlsTestServer ts({"http/1.1"}, [](HttpServerConfig& cfg) { cfg.withTlsKtlsMode(TLSConfig::KtlsMode::Disabled); });
...
TEST(HttpTlsCompressionStreaming, CompressionActivatedOverThresholdTls) {
  ...
  ts.server.postConfigUpdate([&compressionConfig](HttpServerConfig& cfg) { cfg.withCompression(compressionConfig); });
...
TEST(HttpTlsNegative, LargeResponseFragmentation) {
  ts.setDefault([](const HttpRequestView& req) { return req.makeResponse().body(std::string(300000, 'A')); });
...
TEST(HttpOptionsTraceTls, TraceDisabledOnTlsPolicyRejectsTlsTrace) {
  ...
  ts.setDefault([](const HttpRequestView& req) { return req.makeResponse(http::StatusCodeOK); });
...
test::TlsHttp2TestServer tsH2;

const auto portH2 = tsH2.port();
...
TEST(HttpRangeStatic_H2Tls, ServeCompleteFile_H2Tls) {
  ...
  tsH2.setDefault(StaticFileHandler(tmp.dirPath()));

  test::TlsHttp2Client client(portH2);
...
TEST(HttpTlsKtlsMode, EnabledModeTracksStats) {
  ts.server.postConfigUpdate([](HttpServerConfig& cfg) { cfg.withTlsKtlsMode(TLSConfig::KtlsMode::Enabled); });
  ts.setDefault([](const HttpRequestView& req) { return req.makeResponse("ktls"); });
...
TEST(HttpTlsKtlsMode, DisabledModeDoesNotAttemptKtls) {
  const auto oldStats = ts.stats();
  ts.server.postConfigUpdate([](HttpServerConfig& cfg) { cfg.withTlsKtlsMode(TLSConfig::KtlsMode::Disabled); });
  ts.setDefault([](const HttpRequestView& req) { return req.makeResponse(); });
  ...
  const auto newStats = ts.stats();
  EXPECT_EQ(newStats.ktlsSendEnabledConnections, oldStats.ktlsSendEnabledConnections);
  EXPECT_EQ(newStats.ktlsSendEnableFallbacks, oldStats.ktlsSendEnableFallbacks);
  EXPECT_EQ(newStats.ktlsSendForcedShutdowns, oldStats.ktlsSendForcedShutdowns);
}
...
TEST(HttpTlsKtlsMode, OpportunisticModeEnabledOrFallbackNoForcedClose) {
  ts.server.postConfigUpdate([](HttpServerConfig& cfg) { cfg.withTlsKtlsMode(TLSConfig::KtlsMode::Opportunistic); });
  ts.setDefault([](const HttpRequestView& req) { return req.makeResponse(); });
...
TEST(HttpTlsKtlsMode, ForcedModeEnabledOrForcedShutdown) {
  ts.server.postConfigUpdate([](HttpServerConfig& cfg) { cfg.withTlsKtlsMode(TLSConfig::KtlsMode::Required); });
  ts.setDefault([](const HttpRequestView& req) { return req.makeResponse(); });
...
TEST(HttpTlsKtlsMode, DisabledModeFileServing) {
  ...
  EXPECT_EQ(newStats.ktlsSendBytes, oldStats.ktlsSendBytes);
}
