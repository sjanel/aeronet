...
func main() {
	...
	// Set GOMAXPROCS based on thread count
	...
	if staticDir != "" {
		// serve static via path /static/
		literalRoutes["/static/"] = handleStatic
	}
	...
	topHandler := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		// If static prefix registered, handle /static/... when literalRoutes has /static/
		if strings.HasPrefix(path, "/static/") && staticDir != "" {
			handleStatic(w, r)
			return
		}
