...
- The implementation is flow-control aware: it sends up to the available connection/stream window and continues after receiving `WINDOW_UPDATE` frames (no full in-memory file load). Each stream's send window starts from the peer's settings while its receive window starts from the local settings. Receive credit is replenished in batches at a half-window low-water mark, and closed streams do not receive redundant stream-level updates. Tests: `aeronet/http2/test/http2-connection_test.cpp` (`DataWindowUpdatesAreBatchedAndSkipClosedStream`, `PeerInitialWindowOnlyControlsNewStreamSendWindow`).
- Before reusing a pooled HTTP/2 connection, `HttpClient` non-blockingly processes complete pending control frames. Harmless housekeeping is retained, while GOAWAY, close, malformed, or partial input makes the client reconnect before sending any bytes of the next request. This preserves the non-idempotent retry guarantee without turning normal TLS-encrypted flow-control traffic into a reconnect. Tests: `aeronet/client/test/http-client-http2-e2e_test.cpp` (`PooledReuseDrainsPingAndRejectsGoAway`, `SequentialPostsReuseConnectionWithPendingControlFrames`).
- Completed client response bodies transfer suitably-sized scratch allocations into `HttpResponse`: HTTP/1.1 identity responses retain the receive allocation while exposing only its body suffix, chunked responses transfer their de-framing buffer, decompressed responses transfer codec output, and HTTP/2 responses transfer their DATA accumulator or codec output. An equal-capacity empty replacement remains available for the next exchange; oversized scratch stays client-owned when the next body is small, avoiding duplicate high-water retention. Tests: `aeronet/client/test/response-parser_test.cpp` (`LargeIdentityBodyTransfersReceiveAllocationAndPreservesCapacity`, `ChunkedBodyTransfersAllocationAndPreservesScratchCapacity`, `DecodesEachSupportedEncodingLengthFramed`, `EmptyIdentityBodyKeepsReceiveScratchReusable`), `aeronet/client/test/http-client-core_test.cpp` (`LargeIdentityResponsesTransferReceiveOwnershipAndSurviveReuse`), and `aeronet/client/test/http-client-http2-e2e_test.cpp` (`MissingContentTypeDefaultsToOctetStream`, `LargeResponseBodyReassembledAcrossDataFrames`, `TransparentResponseDecompression`).
- Consecutive ordinary HEADERS frames and DATA payloads up to 256 bytes share the connection output buffer, allowing one TLS write to encrypt a batch of small responses. Larger DATA and oversized HEADERS/CONTINUATION blocks retain their backing allocations in an ordered fragment queue across flow-control stalls and partial writes. Cleartext transports gather fragments with `writev` / `WSASend`.
