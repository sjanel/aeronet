...
- [x] `StaticFileHandler` serves directory trees with zero-copy `sendFile`
...
- **Zero-copy transfers**: regular GET requests use `HttpResponse::sendFile()` so plaintext sockets reuse the kernel
...
- **Config entry point**: the immutable configuration lives in `StaticFileConfig`. A copy is embedded inside
  `HttpServerConfig::staticFiles` so applications can persist the parameters alongside the rest of the server config or
  tweak them via the fluent `withStaticFileConfig()` helper. The handler constructor also accepts a config directly.
...
int main() {
  HttpServerConfig cfg;
  cfg.withPort(8080)
     .withStaticFileConfig([]{
       StaticFileConfig sc;
       sc.defaultIndex = "index.html";
       sc.enableRange = true;
       sc.addEtag = true;
       return sc;
     }());
  ...
  HttpServer server(cfg);
  StaticFileHandler assets("/var/www/html", cfg.staticFiles);
...
The config object is intentionally part of `HttpServerConfig` so higher-level helpers (for example, a future static file
router builder) can reuse the same settings without duplicating option structs. Today you can either pass the struct to
the handler constructor or configure it fluently via `withStaticFileConfig()` and reuse `cfg.staticFiles` when building
handlers.
