...
TEST(HttpCodecCompression, ContentTypeAllowListBlocksCompression) {
  ...
  HttpCodec::TryCompressBody(state, Encoding::gzip, resp);
...
#ifdef AERONET_ENABLE_ZLIB
  EXPECT_EQ(HttpCodec::TryCompressBody(state, Encoding::gzip, resp2), CompressResponseResult::Compressed);
#else
  EXPECT_EQ(HttpCodec::TryCompressBody(state, Encoding::gzip, resp2), CompressResponseResult::Error);
#endif
...
TEST(HttpCodecCompression, GzipNearOverlapTrailerMove_AllCombinationsRoundTrip) {
  ...
  for (uint32_t seed = 1U; seed < 1000U; ++seed) {
    ...
    HttpCodec::TryCompressBody(state, Encoding::gzip, probe);
  ...
  for (bool withCapturedBody : {false, true}) {
    for (bool withTrailers : {false, true}) {
      ...
      if (withCapturedBody) {
        ...
        resp.body(body, http::ContentTypeApplicationOctetStream);
      }
...
TEST(HttpCodecCompression, MaxCompressRatioCanDisableCompression) {
  ...
  HttpCodec::TryCompressBody(state2, Encoding::gzip, resp2);
...
TEST(HttpCodecCompression, ImpossibleCompressionZstd) {
  ...
  HttpResponse resp(body);
  HttpCodec::TryCompressBody(state, Encoding::zstd, resp);
...
TEST(HttpCodecCompression, TryCompressResponseStreamingEarlyExit) {
  ...
  for (Encoding enc : test::SupportedEncodings()) {
    ...
    ASSERT_EQ(HttpCodec::TryCompressBody(state, neg.encoding, resp), CompressResponseResult::ExceedsMaxRatio);
...
TEST(HttpCodecCompression, TryCompressResponseStressWithDifferentScenarios) {
  ...
  static constexpr auto mixedBodyRandomSize = bodySz / 3;
  const auto mixedBody = test::MakeMixedPayload(mixedBodyRandomSize, bodySz - mixedBodyRandomSize);

  CompressionConfig cfg;
  cfg.minBytes = 16U;
  cfg.addVaryAcceptEncodingHeader = true;
  cfg.maxCompressRatio = 0.5F;

  std::string_view compressibleBodyView(compressibleBody);
  std::string_view incompressibleBodyView(incompressibleBody);
  std::string_view mixedBodyView(mixedBody);
...
TEST(HttpCodecCompression, ResponseCompressionStateMakeContext_BehaviorPerEncoder) {
...
#ifdef AERONET_ENABLE_ZLIB
  {
    ...
    auto result = ctx->encodeChunk(plain, produced.capacity(), produced.data());
    ASSERT_FALSE(result.hasError());
