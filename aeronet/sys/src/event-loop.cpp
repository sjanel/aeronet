...
struct IoUring {
  ...
  // Per-fd POLL generation counter.  Incremented on every add/mod/del so stale poll CQEs
  // from a previous poll registration (or a previous incarnation of the same fd number) are
  // discarded during harvest.  This solves the ABA problem where the kernel recycles an fd
  // and a stale CQE passes through the boolean active check.
  uint16_t* fdGen = nullptr;
  // Per-fd DATA-OP generation counter (async recv/send/poll-once).  Only incremented when
  // the fd's incarnation ends (del()/close), never on poll-set updates: a connection may
  // add/mod a multishot poll while a recv/send SQE is in flight (async->sync fallback,
  // CONNECT tunnel transition), and those completions — which hand buffer ownership back —
  // must survive.
  uint16_t* fdIoGen = nullptr;
  ...
  // Thread id of the ring's submitter task (SINGLE_ISSUER binding), captured at ring init.
  pid_t ownerTid = 0;
  // True after cancelAllOps(): every armed op (multishot accept/polls) has been cancelled
  // and prepareForLoopThread() must re-arm them before the next run polls.
  // While set, poll() suppresses its automatic multishot re-arming (accept resubmission and
  // terminated-poll rearm) so the drain actually converges instead of re-arming what the
  // cancellation just tore down.
  bool opsDisarmed = false;
  ...
  // Operations buffered before the ring is created.
  struct DeferredOp {
    int fd;
    uint32_t mask;
    bool isAccept;
  };
  vector<DeferredOp> deferredOps;
};

// Ensure fdGen / fdIoGen / fdMask are large enough to hold index 'fd'.
// Returns false on OOM or invalid fd.
bool EnsureFdGenCap(IoUring* iouring, int fd) {
  ...
  auto* ioGenBuf = static_cast<uint16_t*>(std::realloc(iouring->fdIoGen, newCap * sizeof(uint16_t)));
  if (ioGenBuf == nullptr) [[unlikely]] {
    return false;
  }
  iouring->fdIoGen = ioGenBuf;
  ...
  std::memset(ioGenBuf + oldCap, 0, (newCap - oldCap) * sizeof(uint16_t));
...
constexpr uint32_t kSendMask = 0xEDD5;      // Mnemonic: "sEnDS"  -- async send CQE
constexpr uint32_t kPollOnceMask = 0xB011;  // Mnemonic: "POLLone"  -- one-shot poll CQE
constexpr uint32_t kPollUpdMask = 0xB0DD;   // Mnemonic: "POLLupD"  -- poll_update outcome CQE
...
// Queue SQEs for every staged deferred op (multishot polls + multishot accept).
// Used when the ring is lazily created and when a loop restart re-arms cancelled ops.
void ReplayDeferredOps(IoUring* iouring) {
  for (const auto& op : iouring->deferredOps) {
  ...
  iouring->deferredOps.clear();
  iouring->deferredOps.shrink_to_fit();
}
#elifdef AERONET_LINUX
...
bool EventLoop::add(EventFd event) {
  ...
  // Bump the POLL generation so stale poll CQEs from a previous registration are discarded.
  // Async recv/send CQEs use the separate fdIoGen space and are unaffected.
  ...
  if (!iouring->ringInitialized) {
    ...
    iouring->deferredOps.push_back({event.fd, pollMask, false});
    return true;
  }
...
bool EventLoop::mod(EventFd event) {
  ...
  if (oldMask == newMask) {
    // The kernel poll already has this exact mask: nothing to update.  Skipping is a large
    // win for paths that re-assert interest on every event (e.g. tunnel forwarding), and it
    // avoids needless poll_update races with in-flight completions.
    return true;
  }

  // Normal path: atomically update the existing multishot poll's mask in a single SQE.
  // The generation is bumped so any CQE still in flight from the pre-update poll is
  // discarded at harvest (its mask/user_data no longer describe the current interest).
  ...
  // The update outcome is tracked (kPollUpdMask) instead of being fire-and-forget: the
  // update can race with the target poll firing (-EALREADY) or terminating (-ENOENT), in
  // which case the requested interest would be silently lost and the connection would go
  // deaf (missed writable edges stall tunnels / large responses).  The harvest recovers by
  // installing a fresh multishot poll with the current mask.
  ::io_uring_sqe_set_data64(sqe, PackUserData(event.fd, kPollUpdMask, newGen));
...
void EventLoop::del(NativeHandle fd) {
  ...
  // Bump both generations (poll + data ops) so any in-flight CQEs for this fd are
  ...
  // fdMask is cleared so this fd no longer counts as an active multishot poll
  // (prepareForLoopThread uses fdMask to re-arm polls after a loop-thread change).
...
bool EventLoop::submitAccept(NativeHandle listenFd) {
  ...
  if (!iouring->ringInitialized) {
    // Buffer for replay on first poll().
    iouring->deferredOps.push_back({listenFd, kAcceptMask, true});
    return true;
  }
...
void EventLoop::cancelAccept(NativeHandle listenFd) {
  ...
  // Shut down the listening socket synchronously BEFORE submitting the cancel.
  // shutdown(SHUT_RDWR) on a listening socket atomically:
  //   - removes it from the kernel's listening hash and from any SO_REUSEPORT group,
  //   - aborts the in-flight multishot accept request (it will complete with -ECANCELED),
  //   - aborts any queued connections (sending RST) and rejects new SYNs.
  // This is essential because closing the user-space fd alone is not enough: the
  // multishot accept SQE keeps a kernel reference to the socket file and the listener
  // remains in the REUSEPORT group, accepting connections that the user-space server
  // can no longer service.  That manifests as EADDRINUSE on quick restart and as
  // 10s connect-timeouts when REUSEPORT routes incoming SYNs to a zombie listener.
  // We still submit the cancel below so the multishot accept's SQE is reaped quickly
  // and we may reuse the slot.
  ::shutdown(listenFd, SHUT_RDWR);

  if (!iouring->ringInitialized) {
    // Ring was never created — just remove the deferred accept op.
    for (auto idx = iouring->deferredOps.size(); idx > 0;) {
      --idx;
      if (iouring->deferredOps[idx].isAccept && iouring->deferredOps[idx].fd == listenFd) {
        iouring->deferredOps.erase(iouring->deferredOps.begin() + static_cast<std::ptrdiff_t>(idx));
        break;
      }
    }
    return;
  ...
  if (sqe != nullptr) {
    ...
    // Submit the cancel so the multishot accept SQE is reaped.  shutdown() above
    // has already invalidated the kernel listener state, so the listening port is
    // already free to be reused even if the kernel hasn't yet processed this cancel.
    // The resulting CQE will be harvested by a future poll() iteration and discarded
    // (kCancelSentinel is filtered there).
...
bool EventLoop::submitRecv([[maybe_unused]] NativeHandle fd, [[maybe_unused]] char* buf,
  ...
  auto* iouring = static_cast<IoUring*>(_pRing);
  // Data ops cannot be deferred like polls/accepts: the ring must exist (first poll() ran).
  // Connections are only accepted by a polling loop, so this always holds by construction.
  assert(iouring->ringInitialized);
  ...
  ::io_uring_sqe_set_data64(sqe, PackUserData(fd, kRecvMask, iouring->fdIoGen[fd]));
  return true;
#else
  return false;
#endif
...
bool EventLoop::submitSend([[maybe_unused]] NativeHandle fd, [[maybe_unused]] const char* buf,
                           [[maybe_unused]] std::size_t len, [[maybe_unused]] bool moreToCome) {
#ifdef AERONET_IO_URING
  if (_pRing == nullptr) {
    return false;
  }
  auto* iouring = static_cast<IoUring*>(_pRing);
  assert(iouring->ringInitialized);  // see submitRecv
  if (!EnsureFdGenCap(iouring, fd)) [[unlikely]] {
    return false;
  }
  struct io_uring_sqe* sqe = GetSqe(iouring);
  ...
  // No MSG_DONTWAIT: the kernel arms an internal poll on EAGAIN and retries when the socket
  // becomes writable, so the completion always carries >= 1 byte (or an error) and the caller
  // never has to wait for writability itself.
  ::io_uring_prep_send(sqe, fd, buf, len, MSG_NOSIGNAL | (moreToCome ? MSG_MORE : 0));
  ::io_uring_sqe_set_data64(sqe, PackUserData(fd, kSendMask, iouring->fdIoGen[fd]));
  return true;
#else
  return false;
#endif
...
bool EventLoop::submitPollOnce([[maybe_unused]] NativeHandle fd, [[maybe_unused]] EventBmp mask) {
  ...
  ::io_uring_sqe_set_data64(sqe, PackUserData(fd, kPollOnceMask, iouring->fdIoGen[fd]));
  return true;
#else
  return false;
#endif
...
void EventLoop::prepareForLoopThread() {
#ifdef AERONET_IO_URING
  auto* iouring = static_cast<IoUring*>(_pRing);
  if (iouring == nullptr || !iouring->ringInitialized) {
    return;  // ring is created lazily by the first poll() of this thread
  }
  const bool threadChanged = iouring->ownerTid != ::gettid();
  if (!threadChanged && !iouring->opsDisarmed) {
    return;
  }
  // A previous run's exit drain (cancelAllOps) disarmed every multishot accept/poll — stage
  // the tracked kernel state for re-arming.  Connections were all closed at server stop, so
  // multishot polls for internal fds (wakeup/timer) and the listener accept are the only
  // live registrations.
  iouring->deferredOps.clear();
  for (uint32_t fd = 0; fd < iouring->fdGenCap; ++fd) {
    if (iouring->fdMask[fd] != 0) {
      iouring->deferredOps.push_back({static_cast<int>(fd), iouring->fdMask[fd], false});
    }
  }
  if (iouring->acceptListenFd >= 0) {
    iouring->deferredOps.push_back({iouring->acceptListenFd, kAcceptMask, true});
  }
  iouring->opsDisarmed = false;
  if (threadChanged) {
    // The ring is bound to a previous loop thread (SINGLE_ISSUER / DEFER_TASKRUN / registered
    // ring fd): every io_uring_enter from this thread would fail.  Tear the ring down; the
    // next poll() lazily re-creates it on this thread and replays the staged ops.
    (void)_baseFd.release();  // io_uring_queue_exit closes the ring fd
    ::io_uring_queue_exit(&iouring->ring);
    iouring->ringInitialized = false;
    iouring->hasPendingSqes = false;
    iouring->nbRearmPending = 0;
    return;
  }
  // Same thread: the ring stays; re-arm immediately (SQEs are flushed by the next poll()).
  ReplayDeferredOps(iouring);
#endif
}

void EventLoop::cancelAllOps() {
#ifdef AERONET_IO_URING
  auto* iouring = static_cast<IoUring*>(_pRing);
  if (iouring == nullptr || !iouring->ringInitialized) {
    return;
  }
  // IORING_ASYNC_CANCEL_ANY: cancel every pending operation on the ring (kernel >= 5.19,
  // same floor as the io_uring backend itself).  Required before abandoning the ring at
  // loop-thread exit: io_uring_queue_exit only closes the ring fd, and the kernel context
  // (with its still-armed multishot accept/polls) dies asynchronously — a zombie accept
  // would keep consuming connections from the still-open listener while a rebuilt ring's
  // accept never sees them.
  struct io_uring_sqe* sqe = GetSqe(iouring);
  if (sqe == nullptr) [[unlikely]] {
    log::error("io_uring_get_sqe failed for cancelAllOps");
    return;
  }
  ::io_uring_prep_cancel64(sqe, 0, IORING_ASYNC_CANCEL_ANY);
  ::io_uring_sqe_set_data64(sqe, kCancelSentinel);
  iouring->opsDisarmed = true;
  // Flushed by the next poll() call; the caller drains the resulting terminal CQEs.
#endif
}
...
std::span<const EventLoop::EventFd> EventLoop::poll() {
  ...
  if (!iouring->ringInitialized) {
    const auto sqEntries = std::max(_nbAllocatedEvents * 4U, 1024U);
    const auto cqEntries = std::max(_nbAllocatedEvents * 8U, 4096U);
    struct io_uring_params params{};
    // SINGLE_ISSUER + DEFER_TASKRUN: all submissions come from this thread and completion
    // task work only runs during io_uring_enter(), so ops never execute concurrently with
    // user-space code on the loop thread.  This is both the fastest configuration for a
    // single-threaded reactor (no inter-processor interrupts, no locking on the ring) and
    // a correctness invariant: buffers handed to the kernel (async recv) are only touched
    // while poll() is blocked in io_uring_enter.
    // Deliberately NOT SQPOLL: the kernel-side SQ poller thread spins a full core per ring,
    // which destroys CPU efficiency (measured 2x total CPU for +15% RPS on HTTP keep-alive
    // benchmarks).  Batched submission via submit_and_wait amortizes enter cost just as well.
    ...
    iouring->ownerTid = ::gettid();
    // Register the ring fd so each io_uring_enter skips fdget/fdput on the ring itself.
    // Only valid because the ring is used from a single thread (SINGLE_ISSUER).
    // Best-effort: on failure liburing keeps using the regular fd.
    ::io_uring_register_ring_fd(&iouring->ring);
  ...
  for (uint32_t idx = 0; idx < iouring->nbRearmPending; ++idx) {
    ...
    // Re-arm with the CURRENT tracked mask (a mod() may have changed interest since the
    // termination CQE was generated with the old mask packed in its user_data).
    const auto rearmMask = static_cast<uint32_t>(iouring->fdMask[rearmFd]);
    if (rearmMask == 0) {
      continue;  // fd was deleted meanwhile
    }
    ::io_uring_prep_poll_multishot(sqe, rearmFd, rearmMask);
    ::io_uring_sqe_set_data64(sqe, PackUserData(rearmFd, rearmMask, iouring->fdGen[rearmFd]));
  }
  iouring->nbRearmPending = 0;

  // Submit any batched SQEs and wait for at least one CQE, in a single io_uring_enter().
  // io_uring uses the adaptive-timeout policy's current value as a fixed per-poll timeout; the
  // saturation/idle feedback loop (_timeout.update, below) is epoll/kqueue/WSAPoll-only.
  const int pollTimeoutMs = _timeout.pollTimeoutMs();
  struct __kernel_timespec ts{};
  ts.tv_sec = pollTimeoutMs / 1000;
  ts.tv_nsec = static_cast<long long>(pollTimeoutMs % 1000) * 1000000LL;
  ...
  if (waitRet < 0) {
    ...
    log::error("io_uring_submit_and_wait_timeout failed (timeout_ms={}, err={}, msg={})", pollTimeoutMs, -waitRet,
  ...
  // Returns false on allocation failure (caller stops harvesting; remaining CQEs stay in the CQ).
  const auto ensureOutCapacity = [this, iouring, &out, &nbReady]() {
    if (nbReady < _nbAllocatedEvents) [[likely]] {
      return true;
    }
  ...
  io_uring_for_each_cqe(&iouring->ring, head, cqe) {
    ...
    // ---- Slow path: control CQEs (cancel sentinel, async close, multishot accept, recv/send, poll-once) ----
    ...
    // ---- Async recv / send CQEs (proactor mode) ----
    // recv: EventFd{fd, EventDataArrived, bytesAvailable=cqe->res} — the kernel has already
    //   written cqe->res bytes into the buffer supplied at submitRecv() time.
    // send: EventFd{fd, EventSendComplete, bytesAvailable=cqe->res} — the kernel consumed
    //   cqe->res bytes from the buffer supplied at submitSend() time.
    // Negative res (-errno) is passed through: the dispatcher owns the retry/close decision,
    // and connections being torn down rely on receiving the terminal CQE (including
    // -ECANCELED / -EPIPE) to know the kernel no longer references their buffers.
    if (controlMask == kRecvMask || controlMask == kSendMask) {
      const auto fd = UnpackFd(userData);
      const auto fdIdx = static_cast<uint32_t>(fd);
      // Discard stale CQEs from a recycled fd (data-op generation space).
      if (fdIdx >= iouring->fdGenCap || UnpackGen(userData) != iouring->fdIoGen[fd]) {
        continue;
      }
      if (!ensureOutCapacity()) [[unlikely]] {
        break;
      }
      out[nbReady] = EventFd{fd, controlMask == kRecvMask ? EventDataArrived : EventSendComplete, cqe->res};
      ++nbReady;
      continue;
    }

    // ---- Poll-update outcome CQE ----
    // A failed update means the target poll fired (-EALREADY) or terminated (-ENOENT)
    // while the update was in flight: the requested interest was lost.  Recover by
    // installing a fresh multishot poll with the current mask (the superseded poll, if
    // still armed, carries a stale generation and its CQEs are discarded at harvest).
    if (controlMask == kPollUpdMask) {
      if (cqe->res >= 0) {
        continue;  // update applied
      }
      const auto fd = UnpackFd(userData);
      const auto fdIdx = static_cast<uint32_t>(fd);
      if (fdIdx >= iouring->fdGenCap || UnpackGen(userData) != iouring->fdGen[fd] || iouring->fdMask[fd] == 0) {
        continue;  // a newer add/mod/del superseded this update
      }
      struct io_uring_sqe* sqe = GetSqe(iouring);
      if (sqe == nullptr) [[unlikely]] {
        log::error("io_uring_get_sqe failed for poll_update recovery (fd # {})", fd);
        continue;
      }
      const auto curMask = static_cast<uint32_t>(iouring->fdMask[fd]);
      ::io_uring_prep_poll_multishot(sqe, fd, curMask);
      ::io_uring_sqe_set_data64(sqe, PackUserData(fd, curMask, iouring->fdGen[fd]));
      continue;
    }
  ...
  // Resubmit multishot accept if the kernel terminated it (suppressed while the ring is
  // being disarmed at loop exit — prepareForLoopThread re-arms it for the next run).
  if (acceptNeedsResubmit && iouring->acceptListenFd >= 0 && !iouring->opsDisarmed) {
    struct io_uring_sqe* sqe = GetSqe(iouring);
    if (sqe != nullptr) {
  ...
  // While disarming at loop exit, drop them: prepareForLoopThread re-arms from fdMask.
  iouring->nbRearmPending = iouring->opsDisarmed ? 0 : nbRearm;
