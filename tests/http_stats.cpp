...
TEST(HttpStats, BasicCountersIncrement) {
  ...
  // Single request via shared helper (adds timeout & size cap safety)
  auto respOpt = test_http_client::request(server.port());
  ASSERT_TRUE(respOpt.has_value());
  ASSERT_NE(respOpt->find("200 OK"), std::string::npos);
