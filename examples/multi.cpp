...
int main(int argc, char** argv) {
  ...
  uint16_t threads = 4;  // default
  if (argc > 1) {
    port = static_cast<uint16_t>(std::stoi(argv[1]));
  }
  if (argc > 2) {
    threads = static_cast<uint16_t>(std::stoi(argv[2]));
  }
  ...
  try {
    ...
    MultiHttpServer multi(HttpServerConfig{}.withPort(port).withNbThreads(threads), std::move(router));
