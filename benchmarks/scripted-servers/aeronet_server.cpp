...
template <typename T>
T GetQueryParamOr(const HttpRequest& req, std::string_view key, T defaultValue) {
...
int main(int argc, char* argv[]) {
  ...
  router.setPath(http::Method::GET, "/headers", [](const HttpRequest& req) {
    const std::size_t count = GetQueryParamOr<std::size_t>(req, "count", 10);
    const std::size_t headerSize = GetQueryParamOr<std::size_t>(req, "size", 64);
  ...
  // ============================================================
  // Endpoint 3: /echo - Body echo test
  // Echoes POST body back in response
  // ============================================================
  ...
  router.setPath(http::Method::GET, "/compute", [](const HttpRequest& req) {
    const int complexity = GetQueryParamOr<int>(req, "complexity", 30);
    const int hashIters = GetQueryParamOr<int>(req, "hash_iters", 1000);
  ...
  router.setPath(http::Method::GET, "/json", [](const HttpRequest& req) {
    const std::size_t items = GetQueryParamOr<std::size_t>(req, "items", 10);
    ...
    return HttpResponse(std::move(json), "application/json");
  });
  ...
  router.setPath(http::Method::GET, "/delay", [](const HttpRequest& req) {
    const int delayMs = GetQueryParamOr<int>(req, "ms", 10);
  ...
  router.setPath(http::Method::GET, "/body", [](const HttpRequest& req) {
    const std::size_t size = GetQueryParamOr<std::size_t>(req, "size", 1024);

    return HttpResponse(GenerateRandomString(size));
  });
  ...
  // ============================================================
  // Endpoint 9: /static/* - Static file serving (if --static DIR given)
  // ============================================================
  if (!benchCfg.staticDir.empty()) {
    ...
    if (std::filesystem::is_directory(staticPath)) {
      ...
      staticCfg.enableConditional = true;
