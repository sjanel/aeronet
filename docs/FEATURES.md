...
- The implementation is flow-control aware: it sends up to the available connection/stream window and continues after receiving `WINDOW_UPDATE` frames (no full in-memory file load). Receive credit is replenished in batches at a half-window low-water mark, and closed streams do not receive redundant stream-level updates. Tests: `aeronet/http2/test/http2-connection_test.cpp` (`DataWindowUpdatesAreBatchedAndSkipClosedStream`).
