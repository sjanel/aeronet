#pragma once

#include <string_view>

#include "aeronet/compiler-config.hpp"
#include "aeronet/is-header-whitespace.hpp"

namespace aeronet {

// Trim OWS (optional whitespace) per RFC7230: SP and HTAB only.
AERONET_ALWAYS_INLINE constexpr std::string_view TrimOws(std::string_view sv) noexcept {
  const char* begin = sv.data();
  const char* end = begin + sv.size();

  while (begin < end && http::IsHeaderWhitespace(begin[0])) {
    ++begin;
  }
  while (begin < end && http::IsHeaderWhitespace(end[-1])) {
    --end;
  }

  return {begin, end};
}

// Only trim trailing spaces (no tabs) that can exist for content-length header values after compression (to avoid
// memmove of the full body).
constexpr std::string_view TrimTrailingSpaces(std::string_view sv) noexcept {
  const char* begin = sv.data();
  const char* end = begin + sv.size();

  while (begin < end && end[-1] == ' ') {
    --end;
  }

  return {begin, end};
}

constexpr bool IsOwsTrimmed(std::string_view sv) noexcept {
  return sv.empty() || (!http::IsHeaderWhitespace(sv.front()) && !http::IsHeaderWhitespace(sv.back()));
}

}  // namespace aeronet
