...
// (0 means orderly EOF, negative values are -errno).
// On non-io_uring backends this flag is never set.
inline constexpr EventBmp EventDataArrived = 1U << 29;

// Synthetic flag set on CQEs produced by completion-based send (io_uring IORING_OP_SEND).
// When set, EventFd.fd identifies the connection and EventFd.bytesAvailable contains the
// number of bytes the kernel consumed from the buffer that was supplied via submitSend()
// (negative values are -errno).
// On non-io_uring backends this flag is never set.
inline constexpr EventBmp EventSendComplete = 1U << 28;
...
inline constexpr EventBmp EventPollMask = ~(EventEt | EventAccept | EventDataArrived | EventSendComplete);
