...
TEST(DurationFormatTest, OrderingPreference) {
  ...
  SysDuration orderingDuration = std::chrono::hours(5) + std::chrono::microseconds(10) + std::chrono::minutes(2) +
...
TEST(DurationFormatTest, MultipleYears) {
  SysDuration multiYearsDuration = std::chrono::years(3) + std::chrono::days(10);
  EXPECT_EQ(FormatDuration(multiYearsDuration), "3y10d");
}

TEST(DurationFormatTest, NegativeMicroseconds) {
  SysDuration negativeMicros = -std::chrono::microseconds(15);
  EXPECT_EQ(FormatDuration(negativeMicros), "-15us");
}

TEST(DurationFormatTest, TimePointDiff) {
  SysTimePoint start = SysClock::now();
  SysTimePoint end = start + std::chrono::microseconds(15);
