...
constexpr std::size_t kConnectionPrefaceLength = 24;  // "PRI * HTTP/2.0\r\n\r\nSM\r\n\r\n"
...
Http2Connection::ProcessResult Http2Connection::processInput(std::span<const std::byte> data) {
  ...
  switch (_state) {
    ...
    default:
      return connectionError(ErrorCode::InternalError, "Invalid connection state");
  }
}
...
const Http2Stream* Http2Connection::getStream(uint32_t streamId) const noexcept {
  auto iter = _streams.find(streamId);
  if (iter != _streams.end()) {
    return &iter->second;
  }
...
Http2Stream* Http2Connection::createStream(uint32_t streamId) {
  auto [iter, inserted] = _streams.emplace(streamId, Http2Stream(streamId, _peerSettings.initialWindowSize));
  if (inserted) [[likely]] {
    ++_activeStreamCount;
    return &iter->second;
  }
  log::error("Failed to create stream {}, already existing", streamId);
  return nullptr;
}

void Http2Connection::closeStream(uint32_t streamId, ErrorCode errorCode) {
  auto iter = _streams.find(streamId);
  if (iter == _streams.end()) [[unlikely]] {
    log::error("Attempted to close non-existent stream {}", streamId);
  } else {
    if (!iter->second.isClosed()) {
      log::debug("Will close stream {} with error code {}", streamId, static_cast<uint32_t>(errorCode));
      iter->second.setErrorCode(errorCode);
      --_activeStreamCount;
      if (_onStreamClosed) {
...
void Http2Connection::sendRstStream(uint32_t streamId, ErrorCode errorCode) {
  ...
  if (stream != nullptr) {
    ...
    stream->setErrorCode(errorCode);
    closeStream(streamId, errorCode);
    if (_onStreamReset) {
...
Http2Connection::ProcessResult Http2Connection::handleDataFrame(FrameHeader header,
  ...
  if (!stream->consumeRecvWindow(static_cast<uint32_t>(payloadSize))) {
    return streamError(header.streamId, ErrorCode::FlowControlError, "Stream flow control exceeded");
  }

  ErrorCode err = stream->onRecvData(frame.endStream);
  if (err != ErrorCode::NoError) [[unlikely]] {
...
Http2Connection::ProcessResult Http2Connection::handleHeadersFrame(FrameHeader header,
  ...
  Http2Stream* stream = getStream(header.streamId);
  bool isNewStream = (stream == nullptr);
  ...
  if (isNewStream) {
    ...
    stream = createStream(header.streamId);
    if (stream == nullptr) [[unlikely]] {
      return connectionError(ErrorCode::InternalError, "Failed to create stream");
    }
...
Http2Connection::ProcessResult Http2Connection::handleRstStreamFrame(FrameHeader header,
  ...
  if (stream != nullptr) {
    ...
    stream->setErrorCode(frame.errorCode);
    closeStream(header.streamId, frame.errorCode);
    if (_onStreamReset) {
...
Http2Connection::ProcessResult Http2Connection::handleContinuationFrame(FrameHeader header,
  ...
  Http2Stream* stream = getStream(_headerBlockStreamId);
  if (stream == nullptr) [[unlikely]] {
    return connectionError(ErrorCode::InternalError, "Stream not found for CONTINUATION");
  }

  ErrorCode err = stream->onRecvHeaders(_headerBlockEndStream);
  if (err != ErrorCode::NoError) [[unlikely]] {
  ...
  if (_headerBlockEndStream && stream->isClosed()) {
    closeStream(_headerBlockStreamId);
  }
