...
TEST_F(HttpClientCacheE2ETest, RefetchesAfterTtlExpiry) {
  HttpClient client(HttpClientConfig{}.withCache(std::chrono::milliseconds{50}));
  EXPECT_EQ(client.get(Url("/counter")).value().bodyInMemory(), "1");
  EXPECT_EQ(client.get(Url("/counter")).value().bodyInMemory(), "1");  // still fresh -> cached
  std::this_thread::sleep_for(client.config().cache.refreshPeriod + std::chrono::milliseconds{1});  // exceed the TTL
...
TEST_F(HttpClientCacheE2ETest, CacheShouldBeDisabledForTooLargeRequests) {
  static constexpr std::size_t kLargeBodySize = 1UL << 10U;
  HttpClient client(
      HttpClientConfig{}.withCache(HttpClientConfig::Duration::max()).withCacheMaxRequestSize(kLargeBodySize));
  auto largeRequest = client.makeRequest(http::Method::GET, Url("/a"));

  EXPECT_EQ(client.request(largeRequest).value().bodyInMemory(), "a");
  EXPECT_EQ(client.request(largeRequest).value().bodyInMemory(), "a");

  EXPECT_EQ(_aHits.load(), 1);  // cache should be working for small requests

  std::string largeBody(kLargeBodySize, 'a');
  largeRequest.body(largeBody);

  EXPECT_EQ(client.request(largeRequest).value().bodyInMemory(), "a");
  EXPECT_EQ(_aHits.load(), 2);
  EXPECT_EQ(client.request(largeRequest).value().bodyInMemory(), "a");
  EXPECT_EQ(_aHits.load(), 3);  // cache should not store large requests
}
