...
class Http2Loopback {
 public:
  ...
  void pump(Http2Connection& from, Http2Connection& to) {
  ...
  void feed(Http2Connection& to, std::span<const std::byte> data) {
    ...
    while (!data.empty()) {
      ...
      ADD_FAILURE() << "No progress while feeding input (state=" << ConnectionStateName(to.state()) << ")";
      return;
    }
  }
  ...
  std::vector<HeaderEvent> clientHeaders;
  std::vector<HeaderEvent> serverHeaders;
  std::vector<DataEvent> clientData;
  std::vector<DataEvent> serverData;
  std::vector<GoAwayEvent> clientGoAway;
  std::vector<GoAwayEvent> serverGoAway;
};
...
TEST(Http2Core, DataIsDeliveredToPeer) {
  ...
  ASSERT_EQ(h.client.sendData(streamId, payload, true), ErrorCode::NoError);

  h.pump(h.client, h.server);

  ASSERT_EQ(h.serverData.size(), 1U);
  EXPECT_EQ(h.serverData[0].streamId, streamId);
  EXPECT_TRUE(h.serverData[0].endStream);
  ASSERT_EQ(h.serverData[0].data.size(), payload.size());
  EXPECT_EQ(static_cast<char>(h.serverData[0].data[0]), 'h');
}
...
TEST(Http2Core, SendingMoreThanConnectionSendWindowFails) {
  ...
  std::vector<std::byte> big(static_cast<std::size_t>(65535), std::byte{'a'});
  ASSERT_EQ(h.client.sendData(streamId, big, false), ErrorCode::NoError);
...
TEST(Http2Core, StreamSendWindowIsEnforced) {
  ...
  std::vector<std::byte> payload(static_cast<std::size_t>(1024), std::byte{'x'});
...
TEST(Http2Core, GoAwayReceivedPreventsNewStreamsBeyondLastStreamId) {
  ...
  auto resGo = h.server.processInput(AsSpan(go));
  ASSERT_NE(resGo.action, Http2Connection::ProcessResult::Action::Error);
  EXPECT_EQ(h.server.state(), ConnectionState::GoAwayReceived);
  ...
  auto resHeaders = h.server.processInput(AsSpan(headers));
  EXPECT_EQ(resHeaders.action, Http2Connection::ProcessResult::Action::Continue);
  EXPECT_EQ(h.server.getStream(3), nullptr);
}
...
TEST(Http2Core, InitiateGoAwayQueuesFrameAndUpdatesState) {
  ...
  h.server.initiateGoAway(ErrorCode::NoError, "shutdown");
  EXPECT_EQ(h.server.state(), ConnectionState::GoAwaySent);
...
TEST(Http2Core, RstStreamFromPeerTriggersStreamResetCallback) {
  ...
  h.server.setOnStreamReset([&](uint32_t id, ErrorCode code) {
    resetCalled = true;
    EXPECT_EQ(id, 1U);
    resetCode = code;
  });
  ...
  h.pump(h.server, h.client);
  ASSERT_NE(h.server.getStream(1), nullptr);

  RawBytes rst;
  WriteRstStreamFrame(rst, 1, ErrorCode::Cancel);

  auto res = h.server.processInput(AsSpan(rst));
  ASSERT_NE(res.action, Http2Connection::ProcessResult::Action::Error);
...
TEST(Http2Core, MultipleConcurrentStreamsDeliverHeadersToCorrectStream) {
  ...
  ASSERT_EQ(h.serverHeaders.size(), 5U);
  for (const auto& ev : h.serverHeaders) {
    EXPECT_TRUE(HasHeader(ev, ":method", "GET"));
    EXPECT_TRUE(HasHeader(ev, ":scheme", "https"));
    EXPECT_TRUE(HasHeader(ev, ":authority", "example.com"));
...
TEST(Http2Core, RefusedStreamWhenMaxConcurrentStreamsExceededOnSender) {
  ...
  EXPECT_EQ(h.client.sendHeaders(
                3,
                [&](const HeaderCallback& emit) {
                  emit(":method", "GET");
                  emit(":scheme", "https");
...
TEST(Http2Core, RoundTripManyHeaderSetsClientToServer) {
  ...
  loopback.pump(loopback.client, loopback.server);

  ASSERT_EQ(loopback.serverHeaders.size(), 50U);
  for (int iter = 0; iter < 50; ++iter) {
    const auto& ev = loopback.serverHeaders[static_cast<std::size_t>(iter)];
    EXPECT_TRUE(HasHeader(ev, ":method", "GET"));
    EXPECT_TRUE(HasHeader(ev, ":scheme", "https"));
    EXPECT_TRUE(HasHeader(ev, ":authority", "example.com"));
...
TEST(Http2Core, RoundTripDataChunksAcrossManyFrames) {
  ...
  for (int iter = 0; iter < 20; ++iter) {
    ...
    ASSERT_EQ(h.client.sendData(streamId, payload, endStream), ErrorCode::NoError);
    h.pump(h.client, h.server);
  }

  ASSERT_EQ(h.serverData.size(), 20U);
  for (int iter = 0; iter < 20; ++iter) {
    const auto& ev = h.serverData[static_cast<std::size_t>(iter)];
    EXPECT_EQ(ev.streamId, streamId);
    const bool endStream = (iter == 19);
...
TEST(Http2Core, StreamClosedRejectsDataAfterEndStream) {
  ...
  auto res = h.server.processInput(AsSpan(data));
...
TEST(Http2Core, ManyTinyFramesDontBreakStateMachine) {
  ...
  for (int ii = 0; ii < 100; ++ii) {
    ...
    ASSERT_EQ(h.client.sendData(streamId, b, endStream), ErrorCode::NoError);
    // Pump after each to avoid flow control blocking.
    h.pump(h.client, h.server);
  }

  ASSERT_EQ(h.serverData.size(), 100U);
  EXPECT_TRUE(h.serverData.back().endStream);
}
