...
This directory contains standalone HTTP server executables and `wrk` benchmark scripts to compare **aeronet** against other high-performance HTTP frameworks.
...
```bash
wrk -t4 -c100 -d30s -s lua/large_body.lua http://127.0.0.1:8080/echo
```
...
# Run benchmark
wrk -t4 -c100 -d30s -s lua/static_files.lua http://127.0.0.1:8080/static/index.html
```
...
Use the provided runner script to benchmark all servers and scenarios:
...
```bash
./run_benchmarks.sh --threads 4        # Number of wrk threads (default: nproc/2)
./run_benchmarks.sh --connections 100  # Number of connections (default: 100)
./run_benchmarks.sh --duration 30s     # Test duration (default: 30s)
./run_benchmarks.sh --server aeronet   # Only benchmark specific server
./run_benchmarks.sh --scenario headers # Only run specific scenario
./run_benchmarks.sh --output results/  # Output directory for results

# Run specific scenarios (comma-separated)
./run_benchmarks.sh --scenario headers,body,routing

# Available scenarios: headers, body, static, cpu, mixed, files, routing
```
...
| `/static/*` | GET | Static file serving (aeronet only, with `--static DIR`) |
...
cd benchmarks/scripted-servers/undertow_server
curl -O https://repo1.maven.org/maven2/io/undertow/undertow-core/2.3.10.Final/undertow-core-2.3.10.Final.jar
curl -O https://repo1.maven.org/maven2/org/jboss/xnio/xnio-api/3.8.8.Final/xnio-api-3.8.8.Final.jar
curl -O https://repo1.maven.org/maven2/org/jboss/xnio/xnio-nio/3.8.8.Final/xnio-nio-3.8.8.Final.jar
curl -O https://repo1.maven.org/maven2/org/jboss/logging/jboss-logging/3.5.0.Final/jboss-logging-3.5.0.Final.jar
curl -O https://repo1.maven.org/maven2/org/wildfly/common/wildfly-common/1.6.0.Final/wildfly-common-1.6.0.Final.jar
curl -O https://repo1.maven.org/maven2/org/jboss/threads/jboss-threads/3.4.0.Final/jboss-threads-3.4.0.Final.jar
...
3. **Warm up**: Run a short warmup before the real test
...
4. Register in `run_benchmarks.sh`
...
- [ ] Add memory profiling (heaptrack integration)
- [ ] WebSocket scenarios
