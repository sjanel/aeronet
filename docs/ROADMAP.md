...
- **Response writer: pool compression output buffer** — `RawChars compressedBuffer(...)` in `HttpResponseWriter` heap-allocates on every compressed chunk write. Keep a per-writer reusable compression output buffer (reset size between chunks, never reallocate). Same `BufferCache` pattern already proven in codec layers.
