...
#ifdef AERONET_ENABLE_HTTP2
    if (_config.http2.enable && !state.tlsEstablished && upgrade::DetectUpgradeTarget(request) == ProtocolType::Http2) {
      const auto upgradeValidation = upgrade::ValidateHttp2Upgrade(request);
      if (upgradeValidation.valid) {
        // Generate and send 101 Switching Protocols response
