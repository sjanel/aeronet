...
int main() {
  MyDto myDto{1, "example"};

  HttpResponse resp;
  resp.bodyJson(myDto);   // Content-Type: application/json
  resp.bodyYaml(myDto);   // Content-Type: text/yaml

  // Builder-style chaining (inside a coroutine handler):
  // co_return HttpResponse(200).bodyJson(myDto);
}
```
