...
void SingleHttpServer::sweepIdleConnections() {
  ...
  for (auto cnxIt = _connections.begin(); cnxIt != _connections.end(); ++cnxIt) {
    ...
    // Skip reclaim while an async recv is in flight: the kernel holds a pointer into inBuffer
    // (data() + size()) and a realloc/shrink would move or shrink the buffer beneath it,
    // leading to heap-buffer-overflow when the CQE arrives.
    if (!state.asyncRecvInFlight) {
