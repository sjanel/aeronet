...
class BenchmarkRunner:
  ...
  def __init__(self, args: argparse.Namespace) -> None:
    ...
    self.needs_static = any(self.SCENARIOS[s].requires_static for s in self.scenarios_to_test if s in self.SCENARIOS)
    self.needs_tls = any(self.SCENARIOS[s].requires_tls for s in self.scenarios_to_test if s in self.SCENARIOS)
  ...
  def _detect_repo_script_dir(self) -> Path:
    ...
    return (self.script_dir / ".." / "..").resolve() / "benchmarks" / "scripted-servers"
  ...
  def _resolve_server_filter(self, server_arg: str) -> List[str]:
    ...
    for name in names:
      ...
      if not self._server_available(name):
        raise BenchmarkError(f"Server '{name}' is not available (missing binary or toolchain)")
    return names
  ...
  def _prepare_server_command(self, name: str, extra_args: Optional[Sequence[str]]) -> Tuple[List[str], Optional[Path]]:
    ...
    if name == "python":
      ...
      return [sys.executable or "python3", str(python_script), *extra_args], python_script.parent
    if name == "undertow":
  ...
  def _ensure_go_server_built(self) -> Path:
    ...
    for src in source_candidates:
      ...
      if go_file.is_file():
        ...
        subprocess.run([go_exe, "build", "-o", str(script_binary), str(go_file)], cwd=src, check=True)
        return script_binary
  ...
  def _ensure_rust_server_built(self) -> Path:
    ...
    candidates = [self.script_dir / "rust_server", self.repo_script_dir / "rust_server"]
  ...
  def _ensure_undertow_server_built(self) -> Tuple[Path, str]:
    ...
    if not source_file.is_file() and repo_undertow.joinpath("UndertowBenchServer.java").is_file():
    ...
    if not class_files:
      ...
      subprocess.run([javac, "-cp", classpath, "UndertowBenchServer.java"], cwd=undertow_dir, check=True)
  ...
  def _find_python_server_script(self) -> Path:
    ...
    additional = [self.script_dir / "../../../benchmarks/scripted-servers",
  ...
  def _run_server_suite(self, server: str) -> None:
    ...
    if normal:
      if self._start_server(server, extra_args=None, scheme="http", insecure=False):
    ...
    for scenario in special:
      ...
      if self._start_server(server, extra_args=extra_args, scheme=scheme, insecure=insecure):
        self._run_single(server, scenario)
        self._stop_server(server)
        time.sleep(1)

  def _start_server(self, server: str, *, extra_args: Optional[Sequence[str]], scheme: str, insecure: bool) -> bool:
    ...
    try:
      ...
      return False
    self.server_processes[server] = ProcessHandle(popen=popen, log_fp=log_fp, log_path=log_path, port=port)
  ...
  def _run_single(self, server: str, scenario_name: str) -> None:
    ...
    try:
      ...
      print(f"ERROR: wrk failed for {server} / {scenario_name} (exit {exc.returncode})")
      print(output)
    ...
    self._store_result(server, scenario_name, metrics["rps"], metrics["latency"], metrics["transfer"])
    print(output)
  ...
  def _store_result(self, server: str, scenario: str, rps: str, latency: str, transfer: str) -> None:
    key = (server, scenario)
    self.results_rps[key] = rps
    self.results_latency[key] = latency
    self.results_transfer[key] = transfer

  def _append_result_block(self, server: str, scenario: str, output: str, error: bool) -> None:
  ...
  def _write_result_header(self) -> None:
    sys_info = subprocess.run(["uname", "-a"], capture_output=True, text=True).stdout.strip()
    cpu_info = ""
  ...
  def _print_results_table(self) -> None:
    if not self.results_rps:
      return
    print_results = TablePrinter(self.servers_to_test, self.scenarios_to_test, self.results_rps,
                                 self.results_latency, self.results_transfer)
  ...
  def _write_summary_table(self) -> None:
    ...
    with self.result_file.open("a", encoding="utf-8") as fp:
      ...
      sep = "------------|--------|" + "".join("----------------|" for _ in self.servers_to_test) + "--------"
      fp.write(sep + "\n")
  ...
  def _write_memory_summary_table(self) -> None:
    ...
    with self.result_file.open("a", encoding="utf-8") as fp:
      ...
      fp.write("Scenario       | Server       | RSS       | Peak      | VMHWM     | VMSize    | Swap      \n")
      fp.write("--------------|--------------|-----------|-----------|-----------|-----------|-----------\n")
      for scenario, server, stats in rows:
        fp.write(
  ...
  def _read_memory_stats(self, pid: int) -> Optional[MemoryStats]:
    ...
    for line in status_path.read_text().splitlines():
      ...
      token = value.strip()
      if key == "VmRSS":
        stats.rss_kb = self._parse_kb(token)
      elif key == "VmPeak":
        stats.peak_kb = self._parse_kb(token)
      elif key == "VmHWM":
        stats.hwm_kb = self._parse_kb(token)
      elif key == "VmSize":
        stats.vm_size_kb = self._parse_kb(token)
      elif key == "VmSwap":
        stats.vm_swap_kb = self._parse_kb(token)
      elif key == "Threads":
        stats.threads = self._parse_int(token)
    return stats
  ...
  def _ensure_test_static_files(self) -> None:
    ...
    file_names_and_sizes = [("large.bin", 1 * 1024 * 1024 * 1024), ("medium.bin", 10 * 1024 * 1024)]
    for file_name, size_bytes in file_names_and_sizes:
      target = static_dir / file_name
      if target.is_file():
        continue
      print(f"Creating static test file: {file_name} ({size_bytes // (1024 * 1024)} MB)")
      fallocate = shutil.which("fallocate")
      if fallocate:
        subprocess.run([fallocate, "-l", str(size_bytes), str(target)], check=True)
      else:
  ...
  def _wait_for_server(self, port: int, scheme: str, insecure: bool) -> bool:
    ...
    for _ in range(50):
      try:
        ...
        with urllib.request.urlopen(req, timeout=0.5, context=context if scheme == "https" else None):
          return True
      except Exception:
        time.sleep(0.2)
    return False
...
class TablePrinter:
  def __init__(self, servers: List[str], scenarios: List[str], rps: Dict[Tuple[str, str], str],
               latency: Dict[Tuple[str, str], str], transfer: Dict[Tuple[str, str], str]) -> None:
  ...
  def print_all(self) -> None:
    self._print_box("BENCHMARK RESULTS COMPARISON", "(Requests/sec - higher is better)", self.rps,
                    higher_is_better=True)
    self._print_box("LATENCY COMPARISON", "(Average - lower is better)", self.latency, higher_is_better=False)
    self._print_box("TRANSFER RATE COMPARISON", "(Data throughput - higher is better)", self.transfer,
                    higher_is_better=True)

  def _print_box(self, title: str, subtitle: str, data: Dict[Tuple[str, str], str], higher_is_better: bool) -> None:
    scenario_width = 12
    cell_width = 14
    win_width = 10
    interior = scenario_width + 3 + len(self.servers) * (cell_width + 3) + win_width + 2
    border = "═" * interior
    print("╔" + border + "╗")
    for text in (title, subtitle):
      left = (interior - len(text)) // 2
  ...
  def _best_server(self, scenario: str, data: Dict[Tuple[str, str], str], higher_is_better: bool) -> str:
    cmp_value = None
    best_name = ""
    for srv in self.servers:
      val = data.get((srv, scenario))
      ...
      if numeric is None:
        continue
      if cmp_value is None or (numeric > cmp_value if higher_is_better else numeric < cmp_value):
        cmp_value = numeric
        best_name = srv
    return best_name
  ...
  def _to_numeric(value: str, higher_is_better: bool) -> Optional[float]:
    try:
      ...
      if any(value.endswith(unit) for unit in ("KB", "MB", "GB")):
        suffix = ''.join(ch for ch in value if not ch.isdigit() and ch != '.')
        number = float(''.join(ch for ch in value if ch.isdigit() or ch == '.'))
        scale = {"B": 1, "KB": 1024, "MB": 1_048_576, "GB": 1_073_741_824}.get(suffix, 1)
...
def parse_args() -> argparse.Namespace:
  ...
  parser = argparse.ArgumentParser(description="Run wrk benchmarks across multiple servers")
  parser.add_argument("--threads", type=int, default=default_threads, help="Number of wrk threads")
  parser.add_argument("--connections", type=int, default=default_connections, help="Number of wrk connections")
  parser.add_argument("--duration", type=str, default=default_duration, help="Duration per benchmark (e.g. 30s)")
  parser.add_argument("--warmup", type=str, default=default_warmup, help="Warmup duration (e.g. 5s)")
