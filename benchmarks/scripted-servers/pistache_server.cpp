  ...
  void onRequest(const Pistache::Http::Request& req, Pistache::Http::ResponseWriter response) override {
    ...
    if (method == Pistache::Http::Method::Get && path == "/headers") {
      ...
      for (std::size_t pos = 0; pos < count; ++pos) {
        std::string name = "X-Bench-Header-" + std::to_string(pos);
        std::string value = GenerateRandomString(headerSize);
        response.headers().addRaw(Pistache::Http::Header::Raw(name, value));
      }
      std::ostringstream body;
      body << "Generated " << count << " headers";
      response.send(Pistache::Http::Code::Ok, body.str());
      return;
    }

    if (method == Pistache::Http::Method::Post && path == "/echo") {
      response.send(Pistache::Http::Code::Ok, req.body());
      return;
    }
    ...
    if (method == Pistache::Http::Method::Get && path == "/compute") {
      ...
      std::ostringstream body;
      body << "fib(" << complexity << ")=" << fibResult << ", hash=" << hashResult;
      response.send(Pistache::Http::Code::Ok, body.str());
      return;
    }
    ...
    if (method == Pistache::Http::Method::Get && path == "/json") {
      ...
      json << R"({"items":[)";
      for (std::size_t pos = 0; pos < items; ++pos) {
        if (pos > 0) {
          json << ",";
        }
        json << R"({"id":)" << pos << R"(,"name":"item-)" << pos << R"(","value":)" << (pos * 100) << "}";
      }
      json << "]}";
      response.headers().addRaw(Pistache::Http::Header::Raw("Content-Type", "application/json"));
      response.send(Pistache::Http::Code::Ok, json.str());
      return;
    }
    ...
    if (method == Pistache::Http::Method::Get && path == "/status") {
      std::ostringstream body;
      body << R"({"server":"pistache","threads":)" << gNumThreads << R"(,"status":"ok"})";
      response.headers().addRaw(Pistache::Http::Header::Raw("Content-Type", "application/json"));
      response.send(Pistache::Http::Code::Ok, body.str());
      return;
    }
    ...
    if (method == Pistache::Http::Method::Get && gRouteCount > 0) {
      ...
      if (auto params = ParseUserPostPattern(path)) {
        std::ostringstream body;
        body << "user " << params->userId << " post " << params->postId;
        response.send(Pistache::Http::Code::Ok, body.str());
        return;
      }
      ...
      if (auto params = ParseApiPattern(path)) {
        std::ostringstream body;
        body << "resource " << params->resource << " item " << params->item << " action " << params->action;
        response.send(Pistache::Http::Code::Ok, body.str());
        return;
      }
    }
