...
inline std::string_view FinalizeDecompressedBody(HeadersViewMap& headersMap, HeadersViewMap::iterator encodingHeaderIt,
                                                 std::string_view src, RawChars& buf, std::string_view trailers = {},
                                                 std::size_t* pTrailerStartPos = nullptr) {
  const std::size_t decompressedSizeNbChars = static_cast<std::size_t>(nchars(src.size()));
  buf.ensureAvailableCapacity(decompressedSizeNbChars + trailers.size());

  // Set the new decompressed body AFTER reallocating the buffer above.
  std::string_view body = buf;

  if (!trailers.empty()) {
    *pTrailerStartPos = buf.size();
...
inline bool UseStreamingDecompression(const HeadersViewMap& headersMap,
                                      ...
                                      HeadersViewMap::iterator contentLenIt) {
  if (streamingDecompressionThresholdBytes > 0 && contentLenIt != headersMap.end()) {
...
RequestDecompressionResult HttpCodec::MaybeDecompressRequestBody(const DecompressionConfig& decompressionConfig,
  ...
  RequestDecompressionResult res = DualBufferDecodeLoop(
      [useStreamingDecode = UseStreamingDecompression(
           headersMap, decompressionConfig.streamingDecompressionThresholdBytes, contentLenIt),
  ...
  request._body = FinalizeDecompressedBody(headersMap, encodingHeaderIt, src, bodyAndTrailersBuffer, trailersScratch,
                                           &trailerStartPos);
