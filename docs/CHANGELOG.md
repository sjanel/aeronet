...
## Improvements
- **Per-route server response cache (`AERONET_ENABLE_RESPONSE_CACHE`)**: added a dependency-free, synchronization-free per-server LRU with entry/byte/variant budgets, method + authority + path/query + `Vary` keys, RFC-style Cache-Control freshness/privacy handling, weak ETag / If-None-Match `304` responses, explicit invalidation/statistics, and unified HTTP/1.1, HTTP/2, synchronous, and coroutine-handler integration.
...
- **Zero-copy HTTP/2 client response handoff**: completed identity and decompressed bodies now rotate suitably-sized DATA-assembly or codec scratch allocations into `HttpResponse` instead of making a final body copy. An equal-capacity empty replacement preserves steady-state reuse, while oversized scratch remains client-owned when the next body is small to avoid duplicate high-water retention.
