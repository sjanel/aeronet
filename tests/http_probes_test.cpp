...
TEST(HttpProbes, StartupAndReadinessTransitions) {
  ...
  EXPECT_TRUE(readyResp.find("200") != std::string::npos);

  auto liveResp = aeronet::test::simpleGet(ts.port(), "/livez");
  EXPECT_TRUE(liveResp.find("200") != std::string::npos);
  ...
  EXPECT_TRUE(readyAfterDrain.empty() || readyAfterDrain.find("503") != std::string::npos);
}
...
TEST(HttpProbes, OverridePaths) {
  ...
  EXPECT_TRUE(rResp.find("200") != std::string::npos);
  auto lResp = aeronet::test::simpleGet(ts.port(), "/liv");
  EXPECT_TRUE(lResp.find("200") != std::string::npos);
  auto sResp = aeronet::test::simpleGet(ts.port(), "/start");
  EXPECT_TRUE(sResp.find("200") != std::string::npos);
}
