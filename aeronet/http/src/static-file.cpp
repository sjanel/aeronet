...
[[nodiscard]] bool startsWithCaseInsensitive(std::string_view value, std::string_view prefix) {
  ...
  for (std::size_t charPos = 0; charPos < prefix.size(); ++charPos) {
...
[[nodiscard]] EtagBuf makeStrongEtag(std::uint64_t fileSize, SysTimePoint lastModified) {
  ...
  auto write_hex_direct = [&](std::uint64_t value) -> std::uint8_t {
    if (value == 0) {
      etag.buf[etag.len++] = '0';
      return 1;
    }
    ...
    for (std::int32_t i = static_cast<std::int32_t>(digits) - 1; i >= 0; --i) {
      const auto shift = static_cast<std::uint64_t>(i) * 4U;
  ...
  // file size
  (void)write_hex_direct(fileSize);
  // hyphen
  etag.buf[etag.len++] = '-';
  // nanoseconds timestamp
  (void)write_hex_direct(static_cast<std::uint64_t>(nanos));
  // closing quote
  etag.buf[etag.len++] = '"';
  return etag;
}
...
[[nodiscard]] RangeSelection parseRange(std::string_view raw, std::uint64_t fileSize) {
  ...
  raw = trim(raw);
  if (!startsWithCaseInsensitive(raw, "bytes=")) {
    result.state = RangeSelection::State::Invalid;
    return result;
  }
  raw.remove_prefix(std::strlen("bytes="));
  raw = trim(raw);
  if (raw.empty()) {
    result.state = RangeSelection::State::Invalid;
    return result;
...
StaticFileHandler::StaticFileHandler(std::filesystem::path rootDirectory, StaticFileConfig options)
    : _root(std::move(rootDirectory)), _options(std::move(options)) {
  _options.validate();
  std::error_code ec;
...
bool StaticFileHandler::resolveTarget(const HttpRequest& request, std::filesystem::path& resolvedPath) const {
  ...
  if (isDirectory || requestedTrailingSlash) {
    if (_options.defaultIndex.empty()) {
      return false;
    }
    resolvedPath /= _options.defaultIndex;
    return true;
  }
  return true;
}
...
HttpResponse StaticFileHandler::operator()(const HttpRequest& request) const {
  ...
  if ((_options.addLastModified || _options.enableConditional) && !ec) {
  ...
  EtagBuf etag;
  if ((_options.addEtag || _options.enableConditional) && lastModified != kInvalidTimePoint) {
    etag = makeStrongEtag(fileSize, lastModified);
  }

  std::string_view etagView{etag.buf.data(), etag.len};

  const bool isConditional = _options.enableConditional;
  ...
  if (isConditional) {
    ...
    if (conditionalOutcome.kind == ConditionalOutcome::Kind::PreconditionFailed) {
      ...
      if (_options.addLastModified && lastModified != kInvalidTimePoint) {
  ...
  const bool allowRanges = _options.enableRange && conditionalOutcome.rangeAllowed;
  ...
  if (_options.contentTypeResolver) {
    if (auto resolved = _options.contentTypeResolver(targetPath.generic_string()); !resolved.empty()) {
      resp.contentType(std::move(resolved));
    } else {
      resp.contentType(_options.defaultContentType);
    }
  } else {
    resp.contentType(_options.defaultContentType);
  }
  ...
  if (rangeSelection.state == RangeSelection::State::Valid) {
    ...
    resp.addCustomHeader(http::ContentRange, std::string_view(rangeHeader.buf.data(), rangeHeader.len));
    resp.sendFile(std::move(file), static_cast<std::size_t>(rangeSelection.offset),
