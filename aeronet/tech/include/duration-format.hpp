...
struct std::formatter<::aeronet::PrettyDuration> {
  static constexpr std::pair<std::string_view, ::aeronet::Duration> kDurationUnitsFormat[] = {
  ...
  auto format(::aeronet::PrettyDuration dur, FormatContext &ctx) const -> decltype(ctx.out()) {
    if (dur.dur < ::aeronet::Duration::zero()) {
      ctx.out() = std::format_to(ctx.out(), "-");
      dur.dur = -dur.dur;
    }

    for (int unitPos = 0, unitToPrintPos = 0; unitToPrintPos < nbUnitsToPrint && dur.dur > ::aeronet::Duration::zero();
         ++unitPos) {
