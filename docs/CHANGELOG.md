...
- **HTTP client forward-proxy support (`HttpClientConfig::withProxy`)**: `HttpClient` can now route every request through a cleartext HTTP forward proxy. For an https origin it opens an HTTP `CONNECT` tunnel to the origin through the proxy and completes the origin TLS handshake over the tunnel; for a plain http origin it sends the request to the proxy in absolute-form (`GET http://origin/path HTTP/1.1`, RFC 9112 §3.2.2). The proxy endpoint is given as a URL (`http://host:port`, or a bare `host[:port]` assumed http); an `https` proxy or a malformed URL is a hard setup error (`HttpClientException` at construction). `withProxy(url, caFile)` additionally takes a CA bundle used to verify the origin handshake when an intercepting proxy re-signs origin certificates with its own CA (e.g. mitmproxy) - it overrides the default trust store / `tlsCaFile` for proxied https requests. Connection pooling, keep-alive reuse, redirects and retries all work through the proxy (each CONNECT tunnel is pooled per origin). A proxy that refuses the `CONNECT` surfaces as the new `HttpClientErrc::proxyError`. End-to-end coverage in `aeronet/client/test/http-client-proxy-e2e_test.cpp`.
- **Built-in HTTP client response cache for idempotent requests (`HttpClientConfig::cache`)**: `HttpClient` gained an in-process TTL cache that serves repeated safe requests straight from memory with no network round trip. It is opt-in and off by default - enable it with `withCache(refreshPeriod)` (a zero period keeps it disabled; `Duration::max()` caches for the client's whole lifetime). While a stored response is younger than the refresh period it is returned directly; once older, the next request refetches and refreshes the entry. Entries are keyed by request **method + URL + request headers + body**, so requests that differ in any of those (e.g. a different `Authorization`) never share an entry, and only **2xx** responses for cacheable methods are stored (default `GET` + `HEAD`, restrictable to a subset of `GET`/`HEAD`/`OPTIONS` via `withCacheMethods`; unsafe/non-idempotent methods are rejected by `validate()`). The cache is bounded by `withCacheMaxEntries` (default `1024`): a full cache first prunes expired entries, then evicts the least-recently-refreshed one, and an amortized periodic sweep drops expired entries. `clearResponseCache()` empties it on demand. It is a **pure time-based cache** and deliberately does not interpret HTTP cache-control semantics (`Cache-Control`, `ETag`, `Vary`, `Age`): the refresh period is authoritative. Implemented on a new `HttpMessage::clone()` deep-copy primitive (the response container is otherwise move-only). Coverage in `aeronet/client/test/http-client-cache-e2e_test.cpp` and `aeronet/http/test/http-response_test.cpp` (`Clone*`).
...
- **Dedicated probe listener for built-in Kubernetes probes (`MultiHttpServer`)**: `BuiltinProbesConfig::withDedicatedPort(port)` makes a `MultiHttpServer` serve the `/livez` / `/readyz` / `/startupz` endpoints from an extra single-threaded event loop bound to `port`, instead of inline on the (contended) application port. Because that loop never runs application handlers, probe availability is fully isolated from worker load - so a busy worker no longer risks a probe timeout and an unwanted pod restart, and there is no need to over-provision worker threads to "reserve" one for probes (which `SO_REUSEPORT` never guaranteed anyway). The listener reflects the worker pool: readiness is `200` while any worker accepts traffic (`503` once all drain), startup is `200` once any worker has entered its loop, and liveness is heartbeat-based - each worker publishes a heartbeat at the top of every event-loop iteration, and the pod is `200` unless **every** worker's heartbeat has been stale (loop wedged inside a handler, or otherwise not polling) longer than the new `BuiltinProbesConfig::livenessStaleThreshold` (default `10s`, `withLivenessStaleThreshold()`), so a merely-busy or progressing worker keeps the pod live while a full deadlock trips it (keep the threshold above both your longest handler and the worker `pollInterval`). Fully opt-in and backward compatible: `dedicatedPort == 0` (the default) keeps probes inline on the application port; a standalone `SingleHttpServer` ignores the setting. New `MultiHttpServer::probePort()` accessor and JSON/YAML `builtinProbes.dedicatedPort` / `livenessStaleThreshold` config fields. See [Kubernetes deployment guide](kubernetes-examples.md#isolating-probes-on-a-dedicated-port).

### Bug fixes

- **Out-of-bounds read on a stale poll event (rare macOS-only crash)** - the POSIX connection store indexes its `_activeConnections` / `_activeConnectionStates` vectors by `fd - 1`, and `shrink_to_fit()` trims trailing closed slots so the vector can end below a fd that was closed but for which the poller still delivers a late event. `ConnectionStorage::iterator(fd)` - whose `fd` argument comes straight from the poller and must therefore be total - computed `begin() + (fd - 1)` unconditionally, so such a stale fd made `IsValid()` dereference *past* the vector (the POSIX iterator is a raw pointer, so nothing bounds-checked it). On Linux `epoll_ctl(DEL)` + `close()` reliably purge pending events, so the poller never hands back a closed fd; on macOS a late kqueue event for an already-closed fd has been observed, surfacing as a flaky `Assertion failed: (idx < this->size()) … operator[]` abort in `HttpDrain.StopsNewConnections`. On macOS `iterator(fd)` now maps any out-of-range fd (including invalid / negative fds, which wrap to a huge index) to `end()`, so every `iterator(fd)` + `IsValid()` lookup treats a stale fd as gone - exactly how the event loop already handles one - instead of reading past the vector. The guard is compiled only on macOS (`#ifdef AERONET_MACOS`), since Linux/epoll cannot deliver such a stale fd. Regression test (macOS-only): `ConnectionStorage.StaleFdBeyondShrunkVectorIsReportedGone` reproduces the out-of-bounds read directly (ASAN reports a `heap-buffer-overflow` in `BaseFd::operator bool()` without the fix).
- **Fix compilation error in Debug in aeronet::fullVersionStringView()**:aeronet::fullVersionStringView() (in <aeronet/version.hpp>) builds the version string from function-local static constexpr std::string_view fragments (_sv_name,_sv_version_macro, _sv_glaze_section, …) that are odr-used as reference non-type template parameters of JoinStringView<std::string_view const&...>.
- **Large TLS responses could be truncated by ~one 16 KiB record under load** - the OpenSSL error queue is per-thread, shared by every connection driven by one event-loop thread, and `SSL_get_error()` consults `ERR_peek_error()` *before* the `WANT_READ` / `WANT_WRITE` state. `TlsTransport::read` and the handshake path returned a fatal `Error` *without draining* the OpenSSL error they had inspected (unlike `write`, which already did), so that error lingered on the shared queue - e.g. a plaintext readiness probe hitting the TLS port fails its handshake with `unexpected eof while reading` and left it behind. The *next* connection's benign would-block `SSL_write` then saw the stale error and was misclassified as a fatal `SSL_ERROR_SSL`, making `flushUserSpaceTlsBuffer` drop the still-pending TLS record and drain-close the connection. A large user-space-TLS response (file send without kTLS) thus occasionally arrived short by exactly one record - surfaced as a rare (~1/50) `HttpLargeFile.ServeLargeFileTls` flake where the 16 MiB body came back as 16 760 832 bytes (1023 of 1024 records). `read` and `handshake` now drain (log + consume) the error on every fatal path, so no TlsTransport operation ever leaves the shared queue dirty (verified empirically: OpenSSL leaves nothing queued on non-fatal returns). Regression tests: `TlsTransportTest.Fatal{Handshake,Read}ErrorDrainsSharedErrorQueue` assert the queue is empty after a fatal error.
...
- **HTTP/2 responses could carry a field value with trailing whitespace, rejected by strict peers** - the compression codec pads the `Content-Length` value with trailing spaces so it can rewrite the number in place without shifting the body (`Content-Length: 5979 \r\n`), which HTTP/1.1 tolerates as OWS. Sent over HTTP/2 that padded value was HPACK-encoded verbatim, but RFC 9113 §8.2.1 forbids leading/trailing whitespace in a field value: a strict peer (nghttp2, hence curl) treats the field as malformed and `RST_STREAM`s the stream with `PROTOCOL_ERROR`, so *every compressed response over HTTP/2 failed* for such clients (aeronet's own tolerant client accepted it, masking the issue). The HTTP/2 header encoder now trims OWS from every field value before HPACK-encoding. Regression test: `Http2Connection.ResponseHeaderValuesAreTrimmedOfSurroundingOws`.
- **Empty-body HTTP/1.1 responses omitted `Content-Length`, defeating keep-alive reuse** - a buffered response with no body (e.g. a bare `302` redirect carrying only a `Location` header, or an empty `200`/`404`) was serialized as just headers followed by the blank line, with neither a `Content-Length` nor a `Transfer-Encoding`. Per RFC 7230 §3.3.3 a client must then treat the body as ending at connection close, so a keep-alive client cannot reuse the connection until the idle timeout fires (or, worse, blocks waiting for more bytes). aeronet now synthesizes `Content-Length: 0` for such responses at finalization (as nginx/Apache do), so keep-alive clients frame the zero-length body and reuse the connection immediately. Correctly excluded: body-less statuses that MUST NOT / cannot carry it (`1xx`, `204`, and `304` - RFC 7230 §3.3.2), `HEAD` responses (their body length mirrors the GET, not necessarily `0`), file payloads (an empty file already declares its own `Content-Length: 0`), streaming responses (framed with chunked / a declared length), and direct-compression responses. Regression tests: `HttpResponseTest.EmptyBody*` and `HttpKeepAlive.EmptyBodyResponseCarriesContentLengthZeroAndReusesConnection`.

### Improvements

- Increased maximum length of **reason** in `HttpResponse` from 1024 to 65522.
- **Pre-computed static-file response headers (`StaticFileHandler`)**: the per-file header fragments (`ETag`, `Last-Modified` and the resolved `Content-Type`) are now formatted once and cached, keyed by the resolved file path, so repeated requests to the same, unchanged file no longer re-run the ETag/date/MIME formatting. Cache entries are validated on every request against the file **size** and **modification time** - both read from the single `fstat()` already performed when opening the file - so a modified or replaced file transparently rebuilds its entry and cached headers are never stale. That same change also removes a redundant `last_write_time()` syscall per request: `File` now exposes `lastModified()` captured from its open-time `fstat()`, and the handler uses it instead of a separate stat. The cache is per handler instance (hence per server thread) and lock-free, grows lazily up to the new `StaticFileConfig::headerCacheCapacity` (default **1024**, builder `withHeaderCacheCapacity()`), and is fully disabled with `headerCacheCapacity = 0`; when full, inserting a new file evicts the least-recently-used entry (LRU) so the cache stays bounded while keeping the hot working set resident. A `contentTypeResolver` callback, if installed, is now invoked at most once per (file, stat) instead of on every request.
...
- **HTTP/2 client benchmarks in CI**: the `benchmarks-gh-pages` workflow now runs the scripted *client* suite (aeronet `HttpClient` vs libcurl) over cleartext HTTP/2 (`h2c`) and HTTP/2 over TLS (`h2-tls`) in addition to HTTP/1.1, and publishes a rendered dashboard per protocol to GitHub Pages (`/benchmarks/clients/client_benchmark_h2c.html` and `..._h2-tls.html`) each with a matching shields.io badge. Two new client-benchmark badges (h2c, h2 TLS) were added to the top of the README.
