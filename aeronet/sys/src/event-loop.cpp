...
struct IoUring {
  ...
  // Tracks whether SQPOLL was successfully enabled at ring init.  When true, io_uring_submit
  // is non-blocking (kernel poller drains the SQ asynchronously) and we may need to wait
  // for the SQ to drain via io_uring_sqring_wait when adding many SQEs in a tight loop.
...
struct io_uring_sqe* GetSqe(IoUring* iouring) {
  ...
  if (sqe == nullptr) [[unlikely]] {
    ...
    // Under SQPOLL, io_uring_submit is non-blocking and the kernel poller may not have
    // drained the SQ yet.  Wait for at least one slot to become available.
    if (sqe == nullptr && iouring->sqpollEnabled) [[unlikely]] {
      for (int attempt = 0; attempt < 1024 && sqe == nullptr; ++attempt) {
