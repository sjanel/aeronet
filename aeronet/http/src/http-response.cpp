...
void HttpResponse::setBodyHeaders(std::string_view contentTypeValue, std::size_t newBodySize, BodyHeadersOpts opts) {
  ...
  if (wantCompressionHeaders && oldBodyLenInlined == 0 && _opts.isAddVaryAcceptEncoding()) {
  ...
  if (newBodySize == 0) {
    ...
    auto neededNewSize = newContentTypeHeaderSize + kContentLengthSize + contentEncodingHeaderSize;

    if (addVaryHeader) {
      neededNewSize += HeaderSize(http::Vary.size(), http::AcceptEncoding.size());
    } else if (appendVaryValue) {
      neededNewSize += kVaryHeaderValueSep.size() + http::AcceptEncoding.size();
    }
    ...
    if (oldBodyLen == 0) {
      ...
      if (addContentEncodingHeader) {
        headerAddLine(http::ContentEncoding, contentEncodingValue);
      }

      headerAddLine(http::ContentType, contentTypeValue);
      headerAddLine(http::ContentLength, std::string_view(newBodyLenCharVec.data(), newBodyLenCharVec.size()));
    } else {
      ...
      char* insertPtr = getContentTypeValuePtr();
      insertPtr = Append(contentTypeValue, insertPtr);
...
HttpResponse& HttpResponse::body(std::string_view body, std::string_view contentType) & {
  ...
  if (body.empty()) {
    setBodyHeaders(contentType, 0UL, OverrideContentTypeAndNewBodyIsInline);
  ...
  if (directCompressionEnabled()) {
    ...
    setBodyHeaders(contentType, written, OverrideContentTypeAndNewBodyIsInline);
    if (isHead()) {
      setHeadSize(written);
      return *this;
    }
...
HttpResponse& HttpResponse::bodyAppendRaw(std::string_view body, std::string_view contentType) {
  if (!body.empty()) {
    ...
    int64_t neededCapacity;

    if (oldBodyLen != 0) {
      const uint8_t nCharsOldBodyLen = nchars(oldBodyLen);
      neededCapacity = static_cast<int64_t>(nbCharsNewBodyLen) - static_cast<int64_t>(nCharsOldBodyLen);
      if (!capturedBody) {
      ...
      neededCapacity = static_cast<int64_t>(HeaderSize(http::ContentType.size(), contentType.size())) +
                       static_cast<int64_t>(HeaderSize(http::ContentLength.size(), nbCharsNewBodyLen)) +
