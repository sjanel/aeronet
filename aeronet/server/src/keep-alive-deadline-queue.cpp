...
void KeepAliveDeadlineQueue::swapEntries(std::uint32_t lhsIdx, std::uint32_t rhsIdx) noexcept {
  ...
  _heap[lhsIdx].pState->keepAliveDeadlineIndex = lhsIdx;
  _heap[rhsIdx].pState->keepAliveDeadlineIndex = rhsIdx;
}
...
void KeepAliveDeadlineQueue::remove(ConnectionState& state) {
  ...
  _heap.back().pState->keepAliveDeadlineIndex = ConnectionState::kNoKeepAliveDeadlineIndex;
  _heap.pop_back();
