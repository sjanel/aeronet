...
namespace {
PollTimeoutPolicy FixedPolicy(SysDuration timeout) noexcept {
  return {.baseTimeout = timeout, .minFactor = 1.0F, .maxFactor = 1.0F};
}
}  // namespace
...
TEST(EventLoopTest, AdaptivePollTimeoutIgnoresInterruptedPoll) {
  ...
  // minTimeout=0.0f → 0ms, maxTimeout=8.0f → 8*5=40ms.
  EventLoop loop(PollTimeoutPolicy{.baseTimeout = std::chrono::milliseconds{5}, .minFactor = 0.0F, .maxFactor = 8.0F});
...
TEST(EventLoopTest, AdaptivePollTimeoutSpinsAfterSaturation) {
  ...
  EventLoop loop(PollTimeoutPolicy{.baseTimeout = std::chrono::milliseconds{5}, .minFactor = 0.0F, .maxFactor = 16.0F},
                 2);
  vector<epoll_event> events;
