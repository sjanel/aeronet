...
#define FOLLY_NOINLINE AERONET_NOINLINE
#define FOLLY_LIKELY AERONET_LIKELY
#define FOLLY_UNLIKELY AERONET_UNLIKELY
...
void podCopy(const Pod* b, const Pod* e, Pod* d) {
  ...
  assert(e >= b);
  assert(d >= e || d + (e - b) <= b);
  std::memcpy(d, b, static_cast<std::size_t>(e - b) * sizeof(Pod));
}
