...
RedisEvalRequest RedisSlidingWindowRateLimitStore::buildConsumeRequest(std::string_view key,
  ...
  req.script = kRedisSlidingWindowScript;
  req.scriptSha = kRedisSlidingWindowScriptSha1;
  req.preferEvalSha = _redisConfig.preferEvalSha;
  req.key = buildRedisKey(key);

  const auto nowMs = std::chrono::duration_cast<std::chrono::milliseconds>(now.time_since_epoch()).count();
  const auto windowMs = static_cast<int64_t>(_redisConfig.windowSeconds) * 1000;
  const auto maxInWindow =
      static_cast<int64_t>(config.requestsPerSecond) * static_cast<int64_t>(_redisConfig.windowSeconds);
  ...
  const auto neededSize = nowMsNDigits + windowMsNDigits + maxInWindowNDigits;
...
std::optional<RateLimitDecision> RedisSlidingWindowRateLimitStore::parseConsumeResponse(
  ...
  if (!response.ok) {
    return config.failOpen ? std::optional<RateLimitDecision>(RateLimitDecision::Allow())
                           : std::optional<RateLimitDecision>(RateLimitDecision::Reject(1));
  }
