...
- Completed HTTP/2 client response bodies transfer suitably-sized DATA-assembly or decompression scratch allocations into `HttpResponse`, eliminating the final body copy while preserving an equal-capacity accumulator for the next exchange. Oversized scratch remains client-owned when the next body is small, avoiding duplicate high-water retention. Tests: `aeronet/client/test/http-client-http2-e2e_test.cpp` (`MissingContentTypeDefaultsToOctetStream`, `LargeResponseBodyReassembledAcrossDataFrames`, `TransparentResponseDecompression`).
