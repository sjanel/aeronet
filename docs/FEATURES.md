...
- `HttpServerConfig::connectAllowlist` - list of allowed target hosts (exact, case-insensitive string match). CONNECT is
  ...
  `withConnectAllowlist()` builder helper. The special entry `"*"` allows every host and port, restoring the previous
  unrestricted behavior. This includes loopback, private-network, link-local, and cloud metadata targets, so only use
  `"*"` when unrestricted proxy behavior is intentional and independently access-controlled. Ports are not part of a
  normal allowlist entry, so an explicitly allowed host can be reached on any valid numeric port.
...
- The tunneling path prioritizes a dedicated `tunnelOutBuffer` to avoid mixing HTTP response buffering semantics with
  ...
  reason about.
- Tests: Basic coverage includes successful echo tunneling, DNS resolution failures, exact and wildcard allowlisting,
  and fail-closed default behavior. See [http-connect_test.cpp](../tests/http-connect_test.cpp) and
...
- [x] Non-virtual transport dispatch stores `PlainTransport` directly in the final `Transport` owner, eliminating its per-connection allocation while keeping direct read/write and capability paths without a vptr. TLS and custom/test backends remain out of line and use a cached operation table. Exact backend access uses `Transport::get<T>()` instead of RTTI, and builds without OpenSSL do not instantiate or link the TLS backend. On 64-bit Linux, `PlainTransport` is 16 bytes and `Transport` is 24 bytes. Tests: `aeronet/sys/test/transport_test.cpp`, `aeronet/tls/test/tls-components_test.cpp`; benchmark: `aeronet-bench-internal-transport-dispatch`.
...
- [x] `io_uring` event loop backend *(Linux-only, kernel ≥ 5.19)*. Automatically detected at build time via `AERONET_IO_URING`. Replaces epoll with a completion-based (proactor) design: plain HTTP connections run their whole data path through the ring, so a full event-loop iteration — receives, sends, accepts, closes — costs a single `io_uring_enter` syscall (measured ≈ 0.04 syscalls per request on the `static` benchmark scenario, vs ≥ 2 with epoll). Key features:
  - **Ring setup**: `IORING_SETUP_SINGLE_ISSUER | IORING_SETUP_DEFER_TASKRUN` (fallback `COOP_TASKRUN` on older kernels) with the ring fd registered via `io_uring_register_ring_fd`. Ring creation is deferred to the first `poll()` so SINGLE_ISSUER binds to the event-loop thread. DEFER_TASKRUN doubles as a correctness invariant: ring ops only execute inside `io_uring_enter`, never concurrently with user-space code on the loop thread. Deliberately **no SQPOLL** — the kernel SQ-poller thread spins a full core per ring, which doubles total CPU for a small RPS gain (measured 2× CPU for +15 % RPS); batched `submit_and_wait` amortizes enter cost without burning a core.
  - **Async recv proactor** (`IORING_OP_RECV`): plain (non-TLS) HTTP connections submit a single-shot `recv` SQE; the kernel deposits incoming bytes directly into the per-connection `inBuffer` (no extra copy, no readiness round-trip, no `read()` syscall) and delivers an `EventDataArrived` CQE. TLS keeps the synchronous read loop because OpenSSL drives the handshake state machine.
  - **Async send proactor** (`IORING_OP_SEND`): HTTP/1.1 responses, HTTP/2 frames (h2c) and WebSocket frames on plain connections are submitted as send SQEs pointing straight into the connection's `outBuffer` (no `write()`/`writev()` syscall, no copy). Short sends resubmit the remainder; the kernel retries internally on would-block so no writability poll is needed. Two-span responses (head + captured body) are sent span-by-span with `MSG_MORE`, coalescing segments without `TCP_CORK` syscalls. Output generated while a send is in flight — pipelined HTTP/1.1 responses via `queueData`, protocol frames via the `ProtocolOutput` drain target — is staged in `pendingOutBuffer` (the kernel owns `outBuffer` until the CQE) and promoted on completion, preserving ordering for pipelining, `100-continue`, streaming and frame sequencing. Tunnels, TLS and `MSG_ZEROCOPY` transports stay on the synchronous write path.
  ...
  - **Readiness-based poll** (`IORING_OP_POLL_ADD` multishot): used for TLS connections, tunnels, and internal fds (wakeup/timer). Two per-fd 16-bit generation spaces provide ABA safety: poll registrations bump on every add/mod/del (superseded poll CQEs are discarded), while async recv/send completions use a separate data-op generation that only changes on close — so a poll-set update never invalidates an in-flight recv/send completion. Interest updates use `IORING_OP_POLL_UPDATE` with a tracked outcome: an update racing with the target poll firing (`-EALREADY`) or terminating (`-ENOENT`) is recovered by re-arming a fresh poll with the current mask (a silent loss here stalls tunnels / large responses on missed writable edges). Same-mask updates are skipped entirely.
  - **Thread-safe restart**: the ring is bound to its first submitter thread (SINGLE_ISSUER). At loop exit every armed op is cancelled and drained on the owning thread (`EventLoop::cancelAllOps`; terminal CQEs hand buffer ownership back, and connections accepted during the drain are kept for the next run), and `EventLoop::prepareForLoopThread()` re-arms the tracked state on the next run's thread — tearing down and lazily re-creating the ring when the thread changed. A stopped server restarted on another thread keeps its bound listener.
  - **SQE batching**: all SQE submissions (recv re-arms, sends, accepts, closes, poll updates) are batched and flushed by a single `io_uring_submit_and_wait_timeout` per loop iteration.
  - **Async close** (`IORING_OP_CLOSE`): connection teardown shuts the socket down synchronously (immediate FIN/RST) and closes the fd through the ring.
  - **Drain-safe teardown**: a connection closed while async recv/send CQEs are in flight is parked (`closePendingAsyncCqe`) with its socket shut down instead of being recycled — the kernel still holds pointers into its buffers, and the release completes when the terminal CQEs are harvested (including a bounded drain at server shutdown). This makes buffer reuse impossible while the kernel can still touch them.
  ...
  - Measured on the scripted-servers `static`/`/ping` scenario (2 threads, 64–256 connections, interleaved A/B): **+29 % requests/s per core and −30 % p50 latency versus the epoll backend** at identical CPU utilization.
  - Tests: full suite passing with io_uring + ASAN. Benchmark: `benchmarks/internal/event-loop-uring_bench.cpp`.
...
- `aeronet.events.processed` – I/O events processed
...
- **`TestTransport`** (`aeronet/test_support/basic/include/aeronet/test-transport.hpp`): In-memory `Transport` implementation wrapping a `TestPipe` with `FaultPolicy` applied. Used for unit-level protocol testing without sockets or event loops.

- **`FaultInjectingTransport`** (`aeronet/test_support/basic/include/aeronet/fault-injecting-transport.hpp`): Decorator that wraps a real `Transport` (e.g., `PlainTransport` on a live socket) and applies `FaultPolicy` to all I/O. Used for integration tests with real sockets and the full event loop.
