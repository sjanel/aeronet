...
- Fix rare connection stall and test timeouts when `epoll_ctl(EPOLL_CTL_MOD)` fails (for example `EACCES` on some ARM/Linux setups) leaving connections with non-empty outbound or tunnel buffers but without EPOLLOUT interest. When `EventLoop::mod()` fails we now clear buffered write state and request drain-and-close so affected connections can be reaped promptly.
