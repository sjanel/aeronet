...
Http2Connection::OutputBlock::OutputBlock(RawBytes&& owner, std::size_t dataOffset, std::size_t dataSize,
  ...
  const auto frameCount =
      static_cast<uint32_t>(dataSize == 0 ? 1 : ((dataSize + maxFrameSize - 1U) / maxFrameSize));
  _remainingSize = dataSize + (static_cast<std::size_t>(frameCount) * FrameHeader::kSize);
  if (frameCount > 1U) {
    _continuationHeaders = RawBytes((static_cast<std::size_t>(frameCount) - 1U) * FrameHeader::kSize);
    _continuationHeaders.setSize(_continuationHeaders.capacity());
  }

  for (uint32_t frameIndex = 0; frameIndex < frameCount; ++frameIndex) {
    const bool first = frameIndex == 0;
    const bool last = frameIndex + 1U == frameCount;
    const FrameType frameType = headers && !first ? FrameType::Continuation
                                                  : (headers ? FrameType::Headers : FrameType::Data);
    uint8_t flags = FrameFlags::None;
    if (headers) {
      flags = first ? ComputeHeaderFrameFlags(endStream, last)
                    : (last ? FrameFlags::ContinuationEndHeaders : FrameFlags::None);
    ...
    WriteFrameHeader(headerData,
                     {static_cast<uint32_t>(framePayloadSize(frameIndex)), frameType, flags, streamId});
  }
}

Http2Connection::OutputBlock Http2Connection::OutputBlock::Data(RawBytes&& owner, std::size_t dataOffset,
                                                                 std::size_t dataSize, uint32_t maxFrameSize,
                                                                 uint32_t streamId, bool endStream) {
  return OutputBlock(std::move(owner), dataOffset, dataSize, maxFrameSize, streamId, endStream, false);
}

Http2Connection::OutputBlock Http2Connection::OutputBlock::Headers(RawBytes&& owner, std::size_t dataOffset,
                                                                    std::size_t dataSize, uint32_t maxFrameSize,
                                                                    uint32_t streamId, bool endStream) {
  return OutputBlock(std::move(owner), dataOffset, dataSize, maxFrameSize, streamId, endStream, true);
}
...
const std::byte* Http2Connection::OutputBlock::frameHeader(uint32_t frameIndex) const noexcept {
  return frameIndex == 0 ? _firstHeader.data()
                         : _continuationHeaders.data() + ((frameIndex - 1U) * FrameHeader::kSize);
}

std::size_t Http2Connection::OutputBlock::getFragments(std::span<std::string_view> fragments) const noexcept {
  if (fragments.empty() || empty()) {
    return 0;
  }
  if (!_framed) {
    fragments[0] = {reinterpret_cast<const char*>(_payload.data() + _payloadOffset + _framePayloadOffset),
  ...
  while (frameIndex < frameCount && fragmentCount < fragments.size()) {
    if (headerOffset < FrameHeader::kSize) {
      fragments[fragmentCount++] = {
          reinterpret_cast<const char*>(frameHeader(frameIndex) + headerOffset), FrameHeader::kSize - headerOffset};
      headerOffset = FrameHeader::kSize;
      if (fragmentCount == fragments.size()) {
        break;
      }
    }
...
std::span<const std::byte> Http2Connection::getPendingOutput() const noexcept {
  ...
  if (getPendingOutputFragments(std::span<std::string_view>(&fragment, 1)) == 0) {
    return {};
  }
  return {reinterpret_cast<const std::byte*>(fragment.data()), fragment.size()};
}

std::size_t Http2Connection::getPendingOutputFragments(std::span<std::string_view> fragments) const noexcept {
  std::size_t count = 0;
  for (const OutputBlock& block : _outputBlocks) {
    count += block.getFragments(fragments.subspan(count));
    if (count == fragments.size()) {
      return count;
    }
  }
  if (count < fragments.size() && _outputWritePos < _outputBuffer.size()) {
    fragments[count++] = {
        reinterpret_cast<const char*>(_outputBuffer.data() + _outputWritePos), _outputBuffer.size() - _outputWritePos};
  }
  return count;
}
...
void Http2Connection::onOutputWritten(std::size_t bytesWritten) {
  ...
  while (bytesWritten != 0 && !_outputBlocks.empty()) {
    OutputBlock& block = _outputBlocks.front();
    const std::size_t consumed = std::min(bytesWritten, block.remainingSize());
    block.consume(consumed);
    bytesWritten -= consumed;
    if (block.empty()) {
      _outputBlocks.pop_front();
    }
  }
...
void Http2Connection::queueDataBlock(RawBytes&& owner, std::size_t dataOffset, std::size_t dataSize,
                                     uint32_t streamId, bool endStream) {
  queueOutputBlock(
...
ErrorCode Http2Connection::sendData(uint32_t streamId, RawBytes&& owner, std::size_t dataOffset,
...
void Http2Connection::encodeHeaders(uint32_t streamId, http::StatusCode statusCode, HeadersView headersView,
  ...
  // The header encoder owns the trailing buffer exclusively (sendHeaders seals older output first).
  // Move that allocation into a framed block and interleave HEADERS / CONTINUATION headers with views
  // over the original HPACK bytes. No HPACK bytes are moved or copied.
