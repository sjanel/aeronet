...
constexpr std::size_t kContentLengthValueWidth = HttpResponse::kContentLengthValueWidth;
...
constexpr char* WriteCRLFContentLength(char* insertPtr, std::size_t bodySize) {
  ...
  const auto [ptr, ec] = std::to_chars(insertPtr, insertPtr + kContentLengthValueWidth, bodySize);
  assert(ec == std::errc{});
  std::memset(ptr, ' ', static_cast<std::size_t>(insertPtr + kContentLengthValueWidth - ptr));
  return insertPtr + kContentLengthValueWidth;
}
...
inline constexpr auto kContentLengthSize =
    HttpResponse::HeaderSize(http::ContentLength.size(), HttpResponse::kContentLengthValueWidth);

inline constexpr auto kTransferEncodingSizeDiff =
    static_cast<int64_t>(kTransferEncodingChunkedCRLF.size()) - static_cast<int64_t>(kContentLengthSize);

constexpr std::size_t NeededBodyHeadersSize(std::size_t bodySize, std::size_t contentTypeSize) {
  if (bodySize == 0) {
    return 0;
  }
  return HttpResponse::HeaderSize(http::ContentType.size(), contentTypeSize) + kContentLengthSize;
}
...
HttpResponse::HttpResponse(http::StatusCode code, std::string_view body, std::string_view contentType) {
  ...
  if (body.empty()) {
    ...
    insertPtr = WriteCRLFContentLength(insertPtr, body.size());
...
[[nodiscard]] bool VaryContainsAcceptEncodingToken(std::string_view value) {
  ...
  while (pos <= value.size()) {
    ...
    const std::size_t len = (comma == std::string_view::npos) ? (value.size() - pos) : (comma - pos);
    const std::string_view token = TrimOws(value.substr(pos, len));
    if (!token.empty()) {
...
void HttpResponse::setBodyHeaders(std::string_view contentTypeValue, std::size_t newBodySize,
  ...
  SetBodyEnsureNoTrailers(_opts._trailerLen);
  ...
  const bool addContentEncodingHeader = wantCompressionHeaders && _pCompressionContext == nullptr;

  bool addVaryHeader = false;
  bool appendVaryValue = false;
  if (wantCompressionHeaders && _opts.isAddVaryAcceptEncoding()) {
  ...
  const auto headerAddLineNoEnsure = [this](std::string_view key, std::string_view value) {
    if (key != http::ContentLength) {
      value = TrimOws(value);
    }
  ...
  const auto headerAppendValueNoEnsure = [this, &headerAddLineNoEnsure](std::string_view key, std::string_view value,
                                                                        std::string_view separator) {
    auto optValue = headerValue(key);
    if (!optValue) {
      headerAddLineNoEnsure(key, value);
      return;
    }
  ...
  if (newBodySize == 0) {
    if (oldBodyLen != 0) {
      log::error("SJANEL - setBodyHeaders called with newBodySize == 0 and oldBodyLen != 0, data = \n{}",
    ...
    const auto newBodyLenCharVec = MakePaddedContentLengthValue(newBodySize);
    const auto newContentTypeHeaderSize = HeaderSize(http::ContentType.size(), contentTypeValue.size());
    const std::string_view contentEncodingValue =
        addContentEncodingHeader ? GetEncodingStr(_opts._currentEncoding) : std::string_view{};
    const std::size_t contentEncodingHeaderSize =
        addContentEncodingHeader ? HeaderSize(http::ContentEncoding.size(), contentEncodingValue.size()) : 0UL;
    const std::size_t varyHeaderSize = addVaryHeader ? HeaderSize(http::Vary.size(), http::AcceptEncoding.size()) : 0UL;
    const std::size_t varyAppendSize =
        appendVaryValue ? (kVaryHeaderValueSep.size() + http::AcceptEncoding.size()) : 0UL;
    const auto neededNewSize = newContentTypeHeaderSize + kContentLengthSize + contentEncodingHeaderSize +
                               varyHeaderSize + varyAppendSize + (newBodySize * static_cast<std::size_t>(!isHead()));
    if (oldBodyLen == 0) {
      _data.ensureAvailableCapacityExponential(neededNewSize);
      headerAddLineNoEnsure(http::ContentType, contentTypeValue);
      if (addContentEncodingHeader) {
        headerAddLineNoEnsure(http::ContentEncoding, contentEncodingValue);
      }
      if (addVaryHeader) {
        headerAddLineNoEnsure(http::Vary, http::AcceptEncoding);
      } else if (appendVaryValue) {
        headerAppendValueNoEnsure(http::Vary, http::AcceptEncoding, kVaryHeaderValueSep);
      }
      headerAddLineNoEnsure(http::ContentLength, std::string_view(newBodyLenCharVec.data(), newBodyLenCharVec.size()));
    } else {
      ...
      if (setContentTypeIfPresent) {
        replaceHeaderValueNoRealloc(getContentTypeValuePtr(), contentTypeValue);
      }
...
HttpResponse& HttpResponse::body(std::string_view body, std::string_view contentType) & {
  clearCompressionState();

  if (body.empty()) {
    setBodyHeaders(contentType, 0UL);
    setBodyInternal(std::string_view{});
    _payloadVariant = {};
    return *this;
  }

  if (directCompressionEnabled()) {
    auto& compressionState = *_opts.compressionState();
    const Encoding enc = _opts.expectedEncoding();

    _data.ensureAvailableCapacityExponential(compressionState.maxCompressedSize(enc, body.size()));

    const auto bodyStart = bodyStartPos();
    char* bodyPtr = _data.data() + bodyStart;
    const std::size_t availableCapacity = _data.capacity() - bodyStart;
    const std::size_t written = compressionState.encodeFull(enc, body, availableCapacity, bodyPtr);

    if (written == 0U) [[unlikely]] {
      throw std::runtime_error("Failed to compress response body");
    }

    _opts._currentEncoding = enc;

    setBodyHeaders(contentType, written);
    if (isHead()) {
      setHeadSize(written);
      return *this;
    }

    _data.setSize(bodyStart + written);
    _payloadVariant = {};
    return *this;
  }
...
HttpResponse& HttpResponse::bodyAppendRaw(std::string_view body, std::string_view contentType) {
  if (!body.empty()) {
    ...
    if (hasBodyCaptured()) {
      ...
      if (isHead()) {
        ...
        setHeadSize(_payloadVariant.size() + body.size());
      } else {
...
HttpResponse& HttpResponse::bodyAppend(std::string_view body, std::string_view contentType) & {
  if (_opts._currentEncoding != Encoding::none && _pCompressionContext == nullptr) {
    // TODO: this throw is very annoying, because compression is triggered automatically.
    // It's costly in terms of CPU, but in this case we need to decompress the current body and recompress it with given
    // one. Or we never use a full compression mode, only streaming compression?
    throw std::logic_error("Cannot append body after full compression");
  }
  if (body.empty()) {
    return *this;
  }
  if (_pCompressionContext != nullptr) {
    appendCompressedChunkDirect(body, contentType);
    return *this;
  }

  if (directCompressionEnabled() && !hasBodyCaptured() && !hasBodyFile()) {
    bool isFirstChunk = false;
    startCompressionStream(isFirstChunk);
    appendCompressedChunkDirect(body, contentType);
    return *this;
  }

  return bodyAppendRaw(body, contentType);
}
...
HttpResponse& HttpResponse::file(File fileObj, std::size_t offset, std::size_t length, std::string_view contentType) & {
  ...
  setBodyHeaders(contentType, std::max(1UL, resolvedLength));
  if (resolvedLength == 0) {
...
std::string_view HttpResponse::bodyInMemory() const noexcept {
  ...
  ret.remove_suffix(_opts._trailerLen);
  return ret;
}
...
HttpResponse& HttpResponse::headerAddLine(std::string_view key, std::string_view value) & {
  ...
  const bool isContentLength = CaseInsensitiveEqual(key, http::ContentLength);
  std::array<char, kContentLengthValueWidth> paddedContentLength{};
  if (isContentLength) {
    value = TrimOws(value);
    if (value.size() > kContentLengthValueWidth) [[unlikely]] {
      throw std::invalid_argument("Content-Length value is too large");
    }
    std::memcpy(paddedContentLength.data(), value.data(), value.size());
    if (value.size() < paddedContentLength.size()) {
      std::memset(paddedContentLength.data() + value.size(), ' ', paddedContentLength.size() - value.size());
    }
    value = std::string_view(paddedContentLength.data(), paddedContentLength.size());
  } else {
    value = TrimOws(value);
  }
...
HttpResponse& HttpResponse::trailerAddLine(std::string_view name, std::string_view value) & {
  ...
  */
  ...
  const auto newTrailerLen = SafeCast<decltype(_opts._trailerLen)>(lineSize + _opts._trailerLen);
  ...
  std::size_t neededCapacity = lineSize;
  if (_opts._trailerLen == 0) {
    if (addTrailerHeader) {
      neededCapacity += HeaderSize(http::Trailer.size(), name.size());
    }
    // Add an extra CRLF space for the last CRLF that will terminate trailers in finalize
    neededCapacity += http::CRLF.size();
  } else if (addTrailerHeader) {
    neededCapacity += name.size() + kTrailerValueSep.size();
  }
...
HttpResponseData HttpResponse::finalizeForHttp1(SysTimePoint tp, http::Version version, Options opts,
  ...
  const bool addTrailerHeader = _opts._trailerLen != 0 && opts.isAddTrailerHeader();
  ...
  if (bodySz == 0 || isHeadMethod || hasBodyFile()) {
    ...
    if (isHeadMethod && !hasBodyFile()) {
      if (_opts._trailerLen != 0 && addTrailerHeader) {
        ...
        const auto bodySzStrVec = MakePaddedContentLengthValue(bodySz);
        ...
        _data.setSize(_data.size() - bodySz - _opts._trailerLen);
      }
      _opts._trailerLen = 0;
      _payloadVariant = {};
    }
    ...
    bool moveBodyInline = hasBodyCaptured() && bodySz + _opts._trailerLen <= minCapturedBodySize;
    if (_opts._trailerLen == 0) {
      ...
      int64_t headerSizeDiff = kTransferEncodingSizeDiff;
      ...
      if (hasBodyCaptured()) {
        ...
        int64_t neededSize = static_cast<int64_t>(http::CRLF.size()) + hexDigits + headerSizeDiff;
        if (moveBodyInline) {
          neededSize += static_cast<int64_t>(_payloadVariant.size()) +
                        static_cast<int64_t>(kEndChunkedBody.size() + http::CRLF.size());
        }

        _data.ensureAvailableCapacity(neededSize + static_cast<int64_t>(totalNewHeadersSize));

        char* newDoubleCRLFPtr = _data.data() + static_cast<std::int64_t>(bodyStartPos()) -
                                 static_cast<std::int64_t>(http::DoubleCRLF.size()) +
                                 static_cast<std::int64_t>(totalNewHeadersSize) + headerSizeDiff;
        ...
        if (moveBodyInline) {
          ...
          // trailers
          std::memcpy(insertPtr, bodyAndTrailersView.data() + bodySz, _opts._trailerLen);
          insertPtr += _opts._trailerLen;
        ...
        _data.addSize(static_cast<std::size_t>(neededSize));  // works if neededSize is negative as well
      } else {
        ...
        static constexpr int64_t kMinHeaderSizeDiff = kTransferEncodingSizeDiff;
        ...
        // minimum value of headerSizeDiff is kTransferEncodingSizeDiff (-10)
        // minimum value of hexDigits is 1, so diffSz cannot be negative (DoubleCRLF size = 4, endChunkedBody size = 5)
        static_assert(hex_digits(1) + static_cast<int64_t>(kEndChunkedBody.size()) +
                          static_cast<int64_t>(http::DoubleCRLF.size()) + kMinHeaderSizeDiff >=
                      0);
        std::size_t diffSz =
            hexDigits + kEndChunkedBody.size() + http::DoubleCRLF.size() + static_cast<std::size_t>(headerSizeDiff);

        _data.ensureAvailableCapacity(diffSz + totalNewHeadersSize);
        ...
        const auto newHexStart = static_cast<std::uint64_t>(static_cast<int64_t>(oldBodyStart) +
                                                            static_cast<int64_t>(totalNewHeadersSize) + headerSizeDiff);
        ...
        std::string_view newTrailersFlatView(_data.data() + newTrailersStart, _opts._trailerLen);

        if (newBodyDataStart < oldBodyStart) {
          // New data is located before old data - move body first
          std::memmove(_data.data() + newBodyDataStart, _data.data() + oldBodyStart, bodySz);

          // then move trailers
          std::memmove(_data.data() + newTrailersStart, _data.data() + oldTrailerStart, _opts._trailerLen);
        } else {
          // New body data is located after old data - move trailers first
          std::memmove(_data.data() + newTrailersStart, _data.data() + oldTrailerStart, _opts._trailerLen);
        ...
        Copy(http::CRLF, _data.data() + newTrailersStart + _opts._trailerLen);
...
void HttpResponse::bodyAppendUpdateHeaders(std::string_view givenContentType, std::string_view defaultContentType,
  ...
  assert(_opts._trailerLen == 0);
  const auto oldBodyLen = bodyLength();
  const auto newBodyLenCharVec = MakePaddedContentLengthValue(totalBodyLen);
  if (oldBodyLen == 0) {
    if (givenContentType.empty()) {
      givenContentType = defaultContentType;
    } else {
      givenContentType = CheckContentType(false, givenContentType);
    }
    const auto contentTypeValue = headerValue(http::ContentType);
    if (contentTypeValue) {
      replaceHeaderValueNoRealloc(const_cast<char*>(contentTypeValue->data()), givenContentType);
    } else {
      headerAddLine(http::ContentType, givenContentType);
    }
    const auto contentLengthValue = std::string_view(newBodyLenCharVec.data(), newBodyLenCharVec.size());
    const auto contentLengthHeaderValue = headerValue(http::ContentLength);
    if (contentLengthHeaderValue) {
      replaceHeaderValueNoRealloc(const_cast<char*>(contentLengthHeaderValue->data()), contentLengthValue);
    } else {
      headerAddLine(http::ContentLength, contentLengthValue);
    }
  } else {
    if (!givenContentType.empty()) {
...
void HttpResponse::bodyAppendUpdateHeadersWithOldLen(std::string_view givenContentType,
                                                     std::string_view defaultContentType, std::size_t oldBodyLen,
                                                     std::size_t totalBodyLen) {
  assert(_opts._trailerLen == 0);
  const auto newBodyLenCharVec = MakePaddedContentLengthValue(totalBodyLen);
  if (oldBodyLen == 0) {
...
void HttpResponse::appendCompressedChunkDirect(std::string_view body, std::string_view contentType) {
  if (body.empty()) {
    return;
  }
  if (_pCompressionContext == nullptr) [[unlikely]] {
    throw std::logic_error("Compression context is not initialized");
  }
  if (hasBodyCaptured() || hasBodyFile()) [[unlikely]] {
    throw std::logic_error("Cannot stream-compress into external payload");
  }

  const std::size_t oldBodyLen = internalBodyAndTrailersLen();
  const bool isFirstChunk = oldBodyLen == 0;

  if (contentType.empty()) {
    contentType = http::ContentTypeTextPlain;
  } else {
    contentType = CheckContentType(false, contentType);
  }

  if (isFirstChunk) {
    bodyAppendUpdateHeadersWithOldLen(contentType, http::ContentTypeTextPlain, 0, 0);
  }
  const std::size_t maxCompressed = _pCompressionContext->maxCompressedBytes(body.size());

  _data.ensureAvailableCapacityExponential(maxCompressed);
  char* out = _data.data() + _data.size();
  const std::size_t availableCapacity = _data.capacity() - _data.size();
  const int64_t written = _pCompressionContext->encodeChunk(body, availableCapacity, out);
  ...
  const auto newBodyLen = oldBodyLen + static_cast<std::size_t>(written);
  if (isFirstChunk) {
    const auto newBodyLenCharVec = MakePaddedContentLengthValue(newBodyLen);
    replaceHeaderValueNoRealloc(getContentLengthValuePtr(),
                                std::string_view(newBodyLenCharVec.data(), newBodyLenCharVec.size()));
  } else {
    bodyAppendUpdateHeadersWithOldLen(contentType, http::ContentTypeTextPlain, oldBodyLen, newBodyLen);
  }

  if (oldBodyLen == 0 && !hasContentEncoding()) {
    const std::string_view encodingValue = GetEncodingStr(_opts._currentEncoding);
    headerAddLine(http::ContentEncoding, encodingValue);

    if (_opts.isAddVaryAcceptEncoding()) {
      const auto varyValue = headerValue(http::Vary);
      if (!varyValue) {
        headerAddLine(http::Vary, http::AcceptEncoding);
      } else if (!VaryContainsAcceptEncodingToken(*varyValue)) {
...
void HttpResponse::appendCompressedTailDirect() {
  if (_pCompressionContext == nullptr) {
    return;
  }
  if (hasBodyCaptured() || hasBodyFile()) [[unlikely]] {
    throw std::logic_error("Cannot finalize compression for external payload");
  }

  const std::string_view contentTypeValue = headerValueOrEmpty(http::ContentType);
  std::size_t oldBodyLen = internalBodyAndTrailersLen();
  while (true) {
    _data.ensureAvailableCapacityExponential(_pCompressionContext->endChunkSize());
    char* out = _data.data() + _data.size();
    const std::size_t availableCapacity = _data.capacity() - _data.size();
    const int64_t written = _pCompressionContext->end(availableCapacity, out);
    if (written < 0) [[unlikely]] {
    ...
    _data.addSize(static_cast<std::size_t>(written));
    bodyAppendUpdateHeadersWithOldLen(contentTypeValue, http::ContentTypeTextPlain, oldBodyLen,
                                      oldBodyLen + static_cast<std::size_t>(written));
    oldBodyLen += static_cast<std::size_t>(written);

    if (isHead()) {
      setHeadSize(oldBodyLen);
    }
  }
}
...
void HttpResponse::startCompressionStream(bool& isFirstChunk) {
  isFirstChunk = false;
  assert(directCompressionEnabled());

  const Encoding enc = _opts.expectedEncoding();
  if (_opts._currentEncoding == Encoding::none) {
    auto* compressionState = _opts.compressionState();
    _opts._currentEncoding = enc;
    _pCompressionContext = compressionState->makeContext(enc);
    isFirstChunk = true;
  }
}
