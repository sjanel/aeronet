...
bool SingleHttpServer::bufferProtocolHandlerOutput(IProtocolHandler& handler, ConnectionState& state) {
...
bool SingleHttpServer::processSpecialProtocolHandler(ConnectionIt cnxIt) {
  ...
  while (!pState->inBuffer.empty()) {
    ...
    if (bufferProtocolHandlerOutput(handler, *pState)) {
        ...
        if (IsValid(_connections, it)) {
          ...
          if (state.protocol == ProtocolType::Http2 && state.protocolHandler != nullptr) {
            ...
            if (h2Handler->resumeAsyncTaskByHandle(cb.handle)) {
              ...
              if (bufferProtocolHandlerOutput(*h2Handler, state)) {
                flushOutbound(it);
              }
            }
            continue;
...
void SingleHttpServer::setupHttp2Connection(NativeHandle clientFd, TcpNoDelayMode tcpNoDelayMode,
  ...
  bufferProtocolHandlerOutput(*state.protocolHandler, state);
}
...
SingleHttpServer::CloseStatus SingleHttpServer::handleInH2Tunneling(ConnectionIt cnxIt) {
  ...
  while (true) {
    ...
    if (bufferProtocolHandlerOutput(*h2Handler, peerState)) {
