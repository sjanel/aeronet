...
bool BrotliDecoder::Decompress(std::string_view input, RawChars &out, std::size_t maxDecompressedBytes) {
  ...
  static constexpr std::size_t kChunk = 1 << 16;  // 64 KiB per grow, TODO, make configurable
  while (true) {
    out.ensureAvailableCapacity(kChunk);
    ...
    out.setSize(out.size() + (out.capacity() - out.size() - availOut));
