...
- **`io_uring` event loop backend** *(Linux-only, kernel ≥ 5.19, `AERONET_ENABLE_IO_URING`, auto-detected)*: replaces epoll with a completion-based proactor. Plain (non-TLS) connections run their whole data path through the ring for **HTTP/1.1, HTTP/2 (h2c) and WebSocket** — multishot accept, async recv landing directly in the connection's `inBuffer`, and async send reading directly from `outBuffer` (protocol frames included) — so an entire event-loop iteration (accepts + receives + sends + closes) costs a single `io_uring_enter` syscall (~0.04 syscalls per request measured, vs ≥ 2 with epoll). The ring uses `IORING_SETUP_SINGLE_ISSUER | IORING_SETUP_DEFER_TASKRUN` with a registered ring fd (deliberately no SQPOLL, whose kernel poller thread doubles total CPU for a small throughput gain); DEFER_TASKRUN also guarantees ring ops never run concurrently with loop-thread code, which the buffer-ownership protocol relies on: a connection closed while recv/send completions are in flight is parked until the kernel hands its buffers back, response data queued while a send is in flight is staged and promoted on completion (ordering preserved for pipelining / `100-continue` / streaming), and a server restarted on a new thread cancels + drains its ring at loop exit and transparently rebuilds it on the next run. TLS, tunnels and `MSG_ZEROCOPY` transports keep the synchronous readiness-based path. `HttpClient`'s event loop also runs on the ring and transparently re-binds it when the (externally serialized) client migrates threads between requests. Measured on the scripted-servers `static` scenario (2 threads, interleaved A/B): **≈ +30 % requests/s per core and ≈ −25 % median latency versus the epoll backend** at identical CPU utilization. See `docs/FEATURES.md` for the full design.
